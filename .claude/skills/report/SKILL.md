...
For committed doc surfaces the [`@phoenix/leak-guard`](../../../packages/leak-guard/) PreToolUse hook now enforces this mechanically (issue #173): a write that puts a `/Users/…`, `~/`, `~/.claude`, `~/.usirin`, `~/code/…`, or `/vault/` path into a `*.md`/`.decisions`/`.patterns` file is blocked at write time. This prose is the cheap first line; the guard is the enforcement. The guard can't see the *body you POST to gh* (it's a Bash call, not a Write), so the body still needs your own scrub.
