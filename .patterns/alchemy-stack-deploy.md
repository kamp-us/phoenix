...
- **`state`** — where alchemy stores the deployed-resource state it diffs against. Two choices, and they decide whether dev is offline: `Cloudflare.state()` is a **Cloudflare-hosted** store (reproducible across machines/CI, but its Layer needs credentials + network on *every* run, including `alchemy dev`); `Alchemy.localState()` is a **file-based** store (Layer needs only `FileSystem`/`Path` — no creds, no network). Use `localState()` for the local dev loop (so `alchemy dev` runs offline) and a remote store for CI/shared deploys — e.g. `state: process.env.CI ? Cloudflare.state() : Alchemy.localState()`. **Verified:** with `localState()`, `alchemy dev` booted with no auth prompt and zero external connections; with `Cloudflare.state()` it required `alchemy login`.
...
> **`migrationsDir` ≠ adopting a DB — but phoenix has nothing to adopt.** `migrationsDir` only tells alchemy where the *schema* migrations live, not which physical DB binds. `Cloudflare.D1Database` adopts an existing DB **by name** (it lists and reuses a DB matching the name; no `adopt` flag needed) and `migrationsDir` migrations are idempotent (already-applied skipped, new ones applied — set `migrationsTable: "drizzle_migrations"` for Drizzle compatibility). But phoenix's prod D1 holds **no irreplaceable data** — [ADR 0009](../.decisions/0009-d1-direct-defer-dos-and-workflows.md) is explicit ("D1 is wiped and re-seeded; no live data"), and the seeders are dev-only-gated so prod can't even be populated. So the cutover **creates a fresh D1 and re-seeds** via the import scripts rather than adopting — see the resolved cutover note under [Dev & deploy](#dev--deploy).
...
> **Cutover = go fresh on both axes (resolved; no data to preserve).** A spike confirmed phoenix is effectively greenfield: prod runs but the D1 holds no irreplaceable data ([ADR 0009](../.decisions/0009-d1-direct-defer-dos-and-workflows.md)), and content is re-seedable via the import scripts. So:
> - **Fresh D1** — let `Cloudflare.D1Database` create a new database, apply migrations, then re-run `pnpm sozluk:import` / `pano:import`. (Adoption-by-name is available if ever needed, but unnecessary here.)
...
> Delete the old wrangler-managed `phoenix` worker + `phoenix_db` once the alchemy worker is verified. Reusing the same script name is technically supported (`--adopt` + the tag-recovery path) but strictly higher-risk for zero benefit, since there's no data to keep.
...
> **Verified: session-gated SSE works through the proxy.** A second browser spike confirmed the better-auth session cookie survives the proxy and `/fate/live` validates it via `EventSource(..., {withCredentials:true})` — no proxy or auth config change needed. It works because phoenix's better-auth (1.6.10) emits a dev cookie with **`Secure=false`, no `Domain`, `SameSite=Lax`, `HttpOnly`** (host-only on the Vite origin, stored over `http://localhost`, forwarded through the proxy). Two caveats this surfaced: (a) **don't flip `Secure` on in dev** — setting `NODE_ENV=production`, an `https://` `baseURL`, or `advanced.useSecureCookies:true` makes the cookie un-storable on `http://localhost` and breaks the dev loop; (b) the worker sees `Host: 127.0.0.1:<port>` (the proxy socket), not `<name>.localhost`, so any better-auth `baseURL`/`trustedOrigins` derived from the inbound Host header would read the proxy address in dev — set those explicitly rather than inferring from Host.

### Tests move off `vitest-pool-workers` to `alchemy/Test`

A spike settled the test harness: **`@cloudflare/vitest-pool-workers` cannot run an alchemy worker.** The pool loads the worker module inside workerd, but the alchemy `Cloudflare.Worker` class is a deploy-time *Resource* that transitively imports alchemy's bundler (`rolldown`'s native `.node` binding + Node-only modules) — workerd can't load it (the real workerd entry is a virtual module alchemy generates at bundle time). It's not a config problem (the pool *can* run without `wrangler.jsonc`, via inline miniflare options) — the alchemy worker simply isn't loadable that way.

The working harness (spike-verified, offline) is **`alchemy/Test/Vitest`**: deploy the stack to a local workerd (`dev: true` + `Alchemy.localState()`) in `beforeAll`, then assert over HTTP against `stack.url`. phoenix's existing integration tests port roughly as: `SELF.fetch(...)` → `HttpClient` against `stack.url` (easy); direct `env.PHOENIX_DB.prepare(...)` read-back assertions → black-box HTTP via a worker route (the bulk of the work); and the two DO-internal tests (`runInDurableObject`/alarm, esp. `fate-live.test.ts`) → black-box SSE/timing (the hard cases, no direct equivalent). See the PRD's test-harness slice for sizing.
