...
**`node:sqlite` is banned outright as a test backing:** a test that boots any SQL engine is not a unit test, and `makeSqliteTestDb` / `apps/web/worker/db/sqlite-d1.testing.ts` are **deleted**. Why a faked engine cannot stand in for D1 — and the retired four-tier T0–T3 taxonomy that once blessed one — is [ADR 0082](../.decisions/0082-two-test-tiers-unit-integration.md) (superseding [ADR 0040](../.decisions/0040-testing-taxonomy-and-seam-graduation.md)).
