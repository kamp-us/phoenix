...
| `Compiled.ts` | The shared compiled-definition shapes (`Compiled*Definition`), the identity-keyed `buildSourceResolver` → `CompiledFateSources`, the definition/source `erased→kernel` recoveries, `mapRecord` (the ONE entries→record loop both surfaces map their category records through), `mutationWireType` (the validated-config narrowing — see validation parity below) | Shared internals of both |
...
3. **Run** — `runtime.runPromise` on the runtime, with the request's abort signal (`signal` lives on `ExecutorRequestContext`, the Executor-local extension of the served `FateRequestContext` — the served contract itself carries no abort knob). This is the package's **single Effect→Promise conversion point** (`Effect-TS/effect` LLMS.md § "Integrating Effect into existing applications": fate's `(args) => Promise` resolvers are the non-Effect callback boundary `ManagedRuntime` targets) — oracle-baseline-only since the cutover: the SERVING path's conversion is the platform layer's, outside the package. Because the fiber starts from the runtime, the handler's `Effect.fn` span (wire name; `<Entity>.<capability>` for sources) nests under the runtime's ambient span if one is present — pinned in `Executor.test.ts` with a `Tracer.ParentSpan` collector.
...
```ts
// worker/features/fate/schema.ts (shape pinned in codegen-schema.fixture.ts)
...
- **Validation parity**: the same `collectConfigIssues` walk `FateServer.layer` dies with runs here and **throws `FateServerConfigError` at build time** — duplicate wire names / missing sources / typeless mutations fail the Vite build with the offenders named, before the worker ever boots. Config validation has exactly ONE home (`collectConfigIssues`, the typeless-mutation check included): neither compile surface re-checks; they only re-pin the validated invariant for the type system via `mutationWireType` (Compiled.ts), whose residual throw is an invariant defect (a caller bypassed validation), not config validation.
