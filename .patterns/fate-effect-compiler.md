...
- **Validation parity**: the same `collectConfigIssues` walk `FateServer.layer` dies with runs here and **throws `FateServerConfigError` at build time** — duplicate wire names / missing sources / typeless mutations fail the Vite build with the offenders named, before the worker ever boots. Config validation has exactly ONE home (`collectConfigIssues` — the typeless-mutation check moved there from the two compile surfaces, review B2): neither compile surface re-checks; they only re-pin the validated invariant for the type system via `mutationWireType` (Compiled.ts), whose residual throw is an invariant defect (a caller bypassed validation), not config validation.
