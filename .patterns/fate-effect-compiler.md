...
const response = await handleFate(request, {
	...
	// (during the migration the legacy FateContext fields rode on the SAME object)
});
```
...
A raw bridge-shaped entry (`RawFateOperation` / `RawFateSourceEntry`) passes into the compiled options **verbatim**. This carried the migration's coexistence: the compiled server's `context` factory returns the adapterContext object itself, so a legacy resolver received the **same ctx object** the route passed (identity, not a copy). No raw record exists in phoenix since the v1 cutover (ADR 0042); the identity contract stays pinned (`Executor.test.ts`) because every handler still reads the per-request pair off the one object.
