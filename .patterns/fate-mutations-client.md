...
- **Add (`comment.add` / `definition.add`) — A1: client-originated append, canonical-id dedup.** Write the optimistic node into the nested connection with a temp id (`optimistic:${Date.now()}`). Pass it as the mutation's `optimistic` payload (fate normalizes the temp entity + auto-reconciles it on the result) with `insert: "none"` (a nested list is never a root list), and separately append the temp `EntityId` to the connection's own list state via the public store (`store.getListState`/`setList` off the connection's `ConnectionTag` metadata key) — the phoenix helper the ADR blesses in place of fate's private `insertConnectionEdge`. On the mutation HTTP result, fate's `resolveOptimisticEntity(tempId, serverId)` rewrites the id across every list referencing it (this nested list included) and any server `appendNode` — before or after — then dedups by **canonical entity id** (`replaceListEntityId`'s seen-set / visible-dedup), so temp-node and server-append collapse to one edge. Only a **sub-second transient double** is possible (a server append landing before the HTTP result resolves temp→server); it collapses on resolution, zero persistent divergence. On a rejected add the helper's rollback removes the temp edge (fate clears the temp entity) before the inline error. `useReadbackRefetch` is **kept but narrowed** to the append-loss healer (#714's lost append), not the primary "make my row appear" path. The pano `comment.add` slice ships this behind the default-off `pano-optimistic-comment-add` flag (#1678): the flag-gated payload builder + pure list ops + store-shim are the hook-free core [`apps/web/src/pages/optimisticCommentAdd.ts`](../apps/web/src/pages/optimisticCommentAdd.ts); the call site is `CommentComposer` in [`apps/web/src/pages/PanoPostDetail.tsx`](../apps/web/src/pages/PanoPostDetail.tsx).
...
UI keys off `error.code`, never the message string — the codes are the shared contract with the server's `FateWireCode` annotations ([fate-effect-wire-errors.md](./fate-effect-wire-errors.md)).
