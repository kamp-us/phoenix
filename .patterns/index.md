...
| [effect-testing.md](./effect-testing.md) | The **T0–T3 taxonomy** ([ADR 0040](../.decisions/0040-testing-taxonomy-and-seam-graduation.md)), the test-kit (`makeSqliteTestDb`/`Database`/`runFateOp`), the `*.unit.test.ts` (T0) convention, the per-test-vs-`it.layer` isolation rule, `@effect/vitest`/`it.effect`; T3 system redirects to `alchemy-test-harness.md` | First stop before writing any test — pick the tier, then the kit |
