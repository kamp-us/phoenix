...
| [feature-flags-agent-workflow.md](./feature-flags-agent-workflow.md) | The agent ship-behind-flag → validate → flip → kill → retire workflow — the containment layer for no-eyeball autonomous shipping; runtime flag gate vs merge-time control-plane boundary (both stand); copy-pasteable `Flags`/`FlagGate`/`FlagshipFlag` snippets | Shipping a feature dark behind a flag through the autonomous pipeline; deciding how/when to flip or kill ([ADRs 0081](../.decisions/0081-feature-flag-substrate-cloudflare-flagship.md) / [0053](../.decisions/0053-control-plane-boundary.md)) |
