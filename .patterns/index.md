...
| [skill-derived-guards.md](./skill-derived-guards.md) | Pinning behaviour that ships as `SKILL.md` prose: parse the skill's literal line (a pure parser + fail-closed resolution + a repo-relative live read + an in-memory drift falsification) instead of hand-mirroring it in TS; the `class-probe` / `step3-contract` instances | Writing a test or guard over logic that lives in a skill's fenced block or step list ([#4054](https://github.com/kamp-us/phoenix/issues/4054)) |
