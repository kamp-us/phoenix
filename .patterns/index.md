...
| [pi-coding-agent-protocol-service.md](./pi-coding-agent-protocol-service.md) | Production coding-agent-backed framed-CBOR service, lease truth, cancellation, and black-box proof boundary | Changing Tuval's default Pi transport, protocol commands, or daily-driver acceptance journey |
...
| [react-flow-canvas.md](./react-flow-canvas.md) | Controlled React Flow state, custom nodes, handles, and free-pan viewport ownership | Building or changing Tuval's session canvas |
...
| [verb-output-pin-surfaces.md](./verb-output-pin-surfaces.md) | The six surfaces a fabrika verb's printed output shape is hand-copied into — contract prose, contract worked examples, the skill's `SKILL.md`, the `Command.withDescription` help string, the README group section, source docblocks — each with an openable example, plus the three ways a sweep misses one; the sweep rule is ADR [0308](../.decisions/0308-bounded-evidence-output-shape.md)'s "contracts move with their verb, in the same PR", and no guard backs it ([#6523](https://github.com/kamp-us/phoenix/issues/6523)) | Changing what a fabrika verb prints, or checking whether a documented output grammar is still true |
