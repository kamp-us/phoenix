...
| [effect-errors.md](./effect-errors.md) | `Schema.TaggedErrorClass` modeling, domain vs infra split, the `WireCode` annotation | Designing a new error or feature's error set |
...
| Doc | Topic | Read when |
|---|---|---|
| [fate-effect-wire-errors.md](./fate-effect-wire-errors.md) | The `@phoenix/fate-effect` package's error contract: the `WireCode` annotation on `Schema.TaggedErrorClass`, `encodeWireError`, the no-registry/one-edit rule, the enumeration pin | Declaring a domain error |
...
| [fate-effect-compiler.md](./fate-effect-compiler.md) | The `@phoenix/fate-effect` compile step — since the v2 cutover (ADR 0043) the differential oracle's BASELINE + the build-time codegen surface, not a serving path, split across modules that never import each other: `Executor.ts` (oracle-only `FateExecutor.toFetchHandler(runtime)`/`compile`, the decode→provide→run→encode resolver pipeline over a `ManagedRuntime` — the package's single conversion point — and identity-keyed `compileFateSources`), `Codegen.ts` (`toCodegenServer(config)` — the build-time inert twin for `schema.ts`/Vite codegen: manifest parity, `InferFateAPI` fidelity via `FateCodegenAPI`, wire-encoded client args, build-time config validation), `Compiled.ts` (shared compiled-definition shapes + the contained erased→kernel narrowings), `RequestContext.ts` (the per-request `FateRequestContext` — pair as VALUES, no `signal` field; abort rides the Executor-local `ExecutorRequestContext` only); the public `FateExecutor` namespace is assembled in the barrel | Touching `Executor.ts`/`Codegen.ts`/`Compiled.ts`/`RequestContext.ts` / the oracle baseline / exporting `schema.ts` for codegen |
