...
| Doc | Topic | Read when |
|---|---|---|
| [fate-effect-wire-errors.md](./fate-effect-wire-errors.md) | The `@kampus/fate-effect` package's error contract: the `ErrorCode` annotation on `Schema.TaggedErrorClass`, `encodeWireError`, the no-registry/one-edit rule, the enumeration pin | Declaring a domain error |
