...
- **fate protocol layer** (the server-side `fate-*` docs) — how the backend serves data. Data views are the schema, `@phoenix/fate-effect` compiles the `Fate.*` record config to a real fate server and runs every handler through the one worker-level `ManagedRuntime`, and `Fate.source` loaders back each view. Served by [fate](https://github.com/usirin/fate)'s native protocol via `createFateServer` mounted on an imperative `HttpRouter.add` route (no tRPC/GraphQL adapter; no Hono).
...
| Doc | Topic | Read when |
|---|---|---|
| [fate-effect-wire-errors.md](./fate-effect-wire-errors.md) | The `@phoenix/fate-effect` package's error contract: the `fateWireCode` annotation on `Schema.TaggedErrorClass`, `encodeWireError`, the no-registry/one-edit rule, the enumeration pin | Declaring a domain error |
...
| [fate-effect-sources.md](./fate-effect-sources.md) | The `@phoenix/fate-effect` package's loader authoring: `Fate.source(ViewClass, {id}, handlers)`, the loader contract (≥1 of `byId`/`byIds`, silent reads, `E = never`, infra failures are defects), inferred `R`, constructor-owned `<Entity>.<capability>` spans | Declaring a source for an entity |
| [fate-effect-operations.md](./fate-effect-operations.md) | The `@phoenix/fate-effect` package's resolver authoring: `Fate.query`/`Fate.list`/`Fate.mutation` pairing a pure-data definition (Schema input/args, success view, declared error union) with an `Effect.fn("<wire name>")` handler; the error-union compile bound; the decode-then-run wrapper (`InputValidationError` → `VALIDATION_ERROR`) | Declaring a query/list/mutation |
...
| [fate-effect-compiler.md](./fate-effect-compiler.md) | The `@phoenix/fate-effect` v1 compile step — since the v2 cutover (ADR 0043) the differential oracle's BASELINE + the build-time codegen surface, not a serving path: `FateExecutor.toFetchHandler(runtime)`/`compile`, the per-request `FateRequestContext` (pair as VALUES; `signal` v1-path-only), the decode→provide→run→encode resolver pipeline over a `ManagedRuntime` (the package's single conversion point, oracle-baseline-only), identity-keyed `compileFateSources`, the contained erased→kernel narrowings; `toCodegenServer(config)` — the build-time inert twin for `schema.ts`/Vite codegen (manifest parity, `InferFateAPI` fidelity via `FateCodegenAPI`, wire-encoded client args, build-time config validation) | Touching `Executor.ts` / the oracle baseline / exporting `schema.ts` for codegen |
| [fate-effect-interpreter.md](./fate-effect-interpreter.md) | The `@phoenix/fate-effect` v2 native plane — THE SERVING PATH since the cutover (ADR 0043): the wire-protocol Schema codecs (`Protocol.ts` — canonical schemas in fate's serialization order + the staged `decodeProtocolRequest` gate with fate's exact rejection messages AND leniency), the `Wire<>` drift pin against fate's exported protocol types, the `FateInterpreter.handleRequest` dispatch loop (unbounded `Effect.forEach`, no owned runtime — the route runs it on the request fiber; spans nest under the router's request span; abort wired at the route edge), the byId walk + batched sources + connection plane, and the differential-oracle harness rules (dual stacks over fresh databases, byte-equal text/status/publishes, no intra-batch read-after-write) | Touching `Protocol.ts`/`Interpreter.ts`/`Walk.ts`/`Connection.ts` / extending the oracle corpus |
| [fate-effect-worker-wiring.md](./fate-effect-worker-wiring.md) | The worker-side composition: the one `fateConfig` (`config.ts`, import-pure, consumed by both the live layer and codegen), `PhoenixFateLive` (`FateServer.layer ⊕ provideMerge(makeFateLayer)`), the init-only `WorkerRuntime` as layer-build vehicle behind the route context layer (lazy first-request build — NO init warmup; async in isolate init hangs workerd), the `/fate` route serving `FateInterpreter.handleRequest` on the request fiber with `interruptOnAbort` abort wiring (ADR 0043), `schema.ts` as `toCodegenServer(fateConfig)`, `runFateOp` (interpreter-backed T2 harness), capability-less sources for unfetchable entities | Wiring the fate server in `apps/web/worker` / touching `config.ts`/`layers.ts`/`route.ts`/`schema.ts`/`run-fate-op.ts` |
| [fate-effect-feature-migration.md](./fate-effect-feature-migration.md) | The per-FEATURE assembly of the `Fate.*` constructors (sozluk is the shipped reference): the file-by-file shape (views→classes, sources→`Fate.source`, records→def+`Effect.fn` pairs, error annotations), the per-request pair, `orDieDrizzle` (infra failures die), `Entity` `Replacements` (relations + Date restating), Schema input/args shapes, the test recipe | Adding a new fate-effect feature (or reading how the migration was shaped) |
...
| [fate-sources.md](./fate-sources.md) | Source conventions: fate never queries D1 (no Drizzle adapter), `byIds` is the workhorse, keyset order lives in the service, the kernel `SourceResolver` background | Wiring a view's reads to a service (conventions; authoring → fate-effect-sources.md) |
| [fate-mutations.md](./fate-mutations.md) | Write conventions: `entity.verb` naming, validation in services, returning the changed entity, delete returns the parent, live publishes | Writing a mutation |
...
| [alchemy-runtime.md](./alchemy-runtime.md) | **Load-bearing.** No _per-request_ `ManagedRuntime` — ONE _worker-level_ runtime as INIT-ONLY layer-build vehicle (built once per isolate, never disposed — CF deviation); `Layer.effectContext` context-sharing into the routes; the `/fate` route yields the interpreter on the request fiber (no runtime on the request path, ADR 0043); where `runtime.runPromise` still legitimately lives (oracle baseline, Node harness) | Touching the fate↔domain seam |
...
| [alchemy-ci-cd.md](./alchemy-ci-cd.md) | The deploy workflow (push→prod, PR→`pr-<n>` preview, close→destroy); `stacks/github.ts` self-provisioning a scoped CI token + repo secrets; the pnpm `exec` flag-forwarding gotcha | Wiring or debugging CI deploys, rotating the CI token |
...
- **No `runtime.runPromise*` outside `@phoenix/fate-effect`'s compile step.** Handlers and source loaders are `Effect.fn` generators paired with pure-data definitions (`Fate.query`/`Fate.list`/`Fate.mutation`/`Fate.source`); the single Effect→Promise conversion lives in `Executor.ts`.
...
- **Testing strategy:** a **T0–T3 taxonomy** ([ADR 0040](../.decisions/0040-testing-taxonomy-and-seam-graduation.md)). T0 pure (`*.unit.test.ts`), T1 a feature service over `node:sqlite`, T2 a fate operation through the compiled server over the full worker layer — all offline in the `unit` Vitest project under `@effect/vitest`. T3 is the deployed-worker stack-smoke (black-box HTTP, [alchemy-test-harness.md](./alchemy-test-harness.md)). Pick the lowest tier that exercises the behavior; domain correctness lives in T1/T2, not T3. See [effect-testing.md](./effect-testing.md).
