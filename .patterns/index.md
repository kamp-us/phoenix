...
| [fate-effect-data-views.md](./fate-effect-data-views.md) | The `@phoenix/fate-effect` package's view authoring: the `FateDataView<Row>()("Name")({fields})` class factory (static `view` IS the kernel dataView), `FateDataView.list` relations, `Entity<typeof View>`, the TS2883 nameability story | Declaring an entity view for a fate-effect (non-bridge) record |
| [fate-effect-sources.md](./fate-effect-sources.md) | The `@phoenix/fate-effect` package's loader authoring: `Fate.source(ViewClass, {id}, handlers)`, the loader contract (≥1 of `byId`/`byIds`, silent reads, `E = never`, infra failures are defects), inferred `R`, constructor-owned `<Entity>.<capability>` spans | Declaring a source for a fate-effect (non-bridge) entity |
| [fate-effect-operations.md](./fate-effect-operations.md) | The `@phoenix/fate-effect` package's resolver authoring: `Fate.query`/`Fate.list`/`Fate.mutation` pairing a pure-data definition (Schema input/args, success view, declared error union) with an `Effect.fn("<wire name>")` handler; the error-union compile bound; the decode-then-run wrapper (`InputValidationError` → `VALIDATION_ERROR`) | Declaring a query/list/mutation for a fate-effect (non-bridge) record |
| [fate-effect-server.md](./fate-effect-server.md) | The `@phoenix/fate-effect` package's composite: the `FateServer` tag + `config` + `layer` (R = handler/source requirements minus the per-request pair), the `CurrentUser`/`LivePublisher` per-request contract (+ the worker-side `livePublisherFor` live implementation: waitUntil scheduling, swallow-with-log, wire shape shared with the bridge bus), init-time validation (duplicate wire names, source completeness), raw legacy record coexistence | Composing a fate-effect server / discharging domain layers |
| [fate-effect-compiler.md](./fate-effect-compiler.md) | The `@phoenix/fate-effect` v1 compile step: `FateExecutor.toFetchHandler(runtime)`/`compile`, the per-request `FateRequestContext` (pair as VALUES + optional abort signal), the decode→provide→run→encode resolver pipeline over the ONE worker `ManagedRuntime` (single conversion point, span nesting per ADR 0041), identity-keyed `compileFateSources`, legacy passthrough by ctx identity, the contained erased→kernel narrowings; `toCodegenServer(config)` — the build-time inert twin for `schema.ts`/Vite codegen (manifest parity, `InferFateAPI` fidelity via `FateCodegenAPI`, wire-encoded client args, build-time config validation) | Wiring the compiled fate server into the worker / touching `Executor.ts` / exporting `schema.ts` for codegen |
| [fate-effect-worker-wiring.md](./fate-effect-worker-wiring.md) | The worker-side v1 composition: the one `fateConfig` (`config.ts`, import-pure, consumed by both the live layer and codegen), `PhoenixFateLive` (`FateServer.layer ⊕ provideMerge(makeFateLayer)`), the one `WorkerRuntime` (lazy first-request layer build — NO init warmup; async in isolate init hangs workerd), the route's `CoexistenceFateContext` (per-request pair + legacy `FateContext` fields on ONE object), `schema.ts` as `toCodegenServer(fateConfig)`, `runFateOp` v2, capability-less sources for unfetchable entities | Wiring the fate server in `apps/web/worker` / touching `config.ts`/`layers.ts`/`route.ts`/`schema.ts`/`run-fate-op.ts` |
| [fate-effect-feature-migration.md](./fate-effect-feature-migration.md) | The per-FEATURE assembly of the `Fate.*` constructors (sozluk is the shipped reference): the file-by-file rewrite (views→classes, sources→`Fate.source`, records→def+`Effect.fn` pairs, error annotations), `CurrentUser`/`LivePublisher` replacing `Auth`/`LiveBus`, `orDieDrizzle` (infra failures die), `Entity` `Replacements` (relations + Date restating), Schema input/args shapes, the test porting recipe | Migrating a feature off the bridge (pano/pasaport) or adding a new fate-effect feature |
...
| [fate-server-wiring.md](./fate-server-wiring.md) | The `/fate` per-request seam + the legacy bridge mechanism (`FateContext` of `{runtime, request, auth, liveBus}`); the assembly half (hand-called `createFateServer`) is superseded by [fate-effect-worker-wiring.md](./fate-effect-worker-wiring.md) | How a legacy bridge record reaches its ctx / the route's session+live seam |
