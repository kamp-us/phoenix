...
The protocol and client layers share one view/type model: the server's `Entity<>` types are the client's types, generated, with no schema artifact between them. The `@phoenix/fate-effect` package ([fate-effect-server.md](./fate-effect-server.md) + [fate-effect-compiler.md](./fate-effect-compiler.md)) is the seam between the domain and protocol layers — read those first when working server-side.
...
Read [fate-effect-server.md](./fate-effect-server.md) + [fate-effect-compiler.md](./fate-effect-compiler.md) first — they're the seam everything else hangs off.
...
| [fate-effect-compiler.md](./fate-effect-compiler.md) | The `@phoenix/fate-effect` v1 compile step: `FateExecutor.toFetchHandler(runtime)`/`compile`, the per-request `FateRequestContext` (pair as VALUES + optional abort signal), the decode→provide→run→encode resolver pipeline over the ONE worker `ManagedRuntime` (single conversion point, span nesting per ADR 0041), identity-keyed `compileFateSources`, the ctx-identity contract, the contained erased→kernel narrowings; `toCodegenServer(config)` — the build-time inert twin for `schema.ts`/Vite codegen (manifest parity, `InferFateAPI` fidelity via `FateCodegenAPI`, wire-encoded client args, build-time config validation) | Wiring the compiled fate server into the worker / touching `Executor.ts` / exporting `schema.ts` for codegen |
| [fate-effect-interpreter.md](./fate-effect-interpreter.md) | The `@phoenix/fate-effect` v2 native plane (oracle-only until the task-17 cutover): the wire-protocol Schema codecs (`Protocol.ts` — canonical schemas in fate's serialization order + the staged `decodeProtocolRequest` gate with fate's exact rejection messages AND leniency), the `Wire<>` drift pin against fate's exported protocol types, the `FateInterpreter.handleRequest` dispatch loop (unbounded `Effect.forEach`, no owned runtime, `encodeWireError` shared with v1), and the differential-oracle harness rules (dual stacks over fresh databases, byte-equal text/status/publishes, no intra-batch read-after-write) | Touching `Protocol.ts`/`Interpreter.ts` / extending the oracle corpus (tasks 15–17) |
| [fate-effect-worker-wiring.md](./fate-effect-worker-wiring.md) | The worker-side v1 composition: the one `fateConfig` (`config.ts`, import-pure, consumed by both the live layer and codegen), `PhoenixFateLive` (`FateServer.layer ⊕ provideMerge(makeFateLayer)`), the one `WorkerRuntime` (lazy first-request layer build — NO init warmup; async in isolate init hangs workerd), the route's per-request `FateRequestContext`, `schema.ts` as `toCodegenServer(fateConfig)`, `runFateOp`, capability-less sources for unfetchable entities | Wiring the fate server in `apps/web/worker` / touching `config.ts`/`layers.ts`/`route.ts`/`schema.ts`/`run-fate-op.ts` |
...
| [alchemy-runtime.md](./alchemy-runtime.md) | **Load-bearing.** No _per-request_ `ManagedRuntime` — ONE _worker-level_ runtime (built once per isolate, never disposed — CF deviation); `Layer.effectContext` context-sharing; how the compile step runs each handler through the runtime | Touching the fate↔domain seam |
