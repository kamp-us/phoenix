...
phoenix's own record of how its fabrika hook layer reached the shape it has. It lived in the plugin's
`docs/` until the portability sweep, which moved it here: every verdict below grades a piece of the
retired v1 pipeline against phoenix's delivery channel, so it resolves in this repo and nowhere else.
The portable half — where a fabrika hook is declared, the harness exit-code contract, the
dispatch-failure policy — stayed in
[`hook-surface.md`](../claude-plugins/fabrika/docs/hook-surface.md), including the record format the
sections below are written in.
...
This page holds two things: the **proven/unproven account** of the ruled delivery channel every verdict grades against, and the **records themselves** — one recorded verdict per graded v1 piece. Each record is written in the format [`hook-surface.md`](../claude-plugins/fabrika/docs/hook-surface.md) specifies; that page is also where the surface being graded — where a fabrika hook is declared and how it invokes a verb — is described.

## What is proven today, and what is not

The ruled channel — a global install of `@kampus/fabrika-cli` whose binary finds the repo root, asks Node's resolver what copy that root installed, and hands the invocation over ([`../packages/fabrika-cli/README.md`](../packages/fabrika-cli/README.md), *How it is delivered*; [`cli-interface-convention.md`](../claude-plugins/fabrika/docs/cli-interface-convention.md) rule 5, *Delivery — one name, two installs, both of them real*) — has **two halves, and only one of them is exercised.**
...
**Graded against:** the retired v1 plugin's `hooks/install.sh` (whole file, 79 lines) · [`../packages/fabrika-cli/README.md`](../packages/fabrika-cli/README.md) *How it is delivered* · [`cli-interface-convention.md`](../claude-plugins/fabrika/docs/cli-interface-convention.md) rule 5 *Delivery* L213–224 · [`../packages/fabrika-cli/src/delegate/node-resolve.ts`](../packages/fabrika-cli/src/delegate/node-resolve.ts) L24–43 · [`../packages/fabrika-cli/src/delegate/resolve.ts`](../packages/fabrika-cli/src/delegate/resolve.ts) L73–74 (the branch an uninstalled root takes). Line numbers true at commit `a2729e6e`.
...
**Graded against:** the retired v1 plugin's `hooks/resolve-data-dir.sh` (whole file, 28 lines) · [`cli-interface-convention.md`](../claude-plugins/fabrika/docs/cli-interface-convention.md) rule 5 L196–198 and *Delivery* L226–232 · [`../packages/fabrika-cli/README.md`](../packages/fabrika-cli/README.md) *How it is delivered*. Line numbers true at commit `a2729e6e`.
...
**Graded against:** the retired v1 plugin's `hooks/pin.sh` (whole file, 22 lines; the pin itself at L22) · [`../package.json`](../package.json) L24 · [`../packages/fabrika-cli/README.md`](../packages/fabrika-cli/README.md) *How it is delivered* · [`cli-interface-convention.md`](../claude-plugins/fabrika/docs/cli-interface-convention.md) rule 5 *Delivery* L210–224 · [`../packages/fabrika-cli/src/delegate/entry.ts`](../packages/fabrika-cli/src/delegate/entry.ts) L77–105 (the branch dispatch) · [`../packages/fabrika-cli/src/delegate/resolve.ts`](../packages/fabrika-cli/src/delegate/resolve.ts) L66–91 (the resolver) and L132–139 (`globalWarning`). Line numbers true at commit `a2729e6e`.
...
**Graded against:** the retired v1 plugin's `hooks/plant-pipeline-link.sh` (whole file, 123 lines; the forcing constraint at L6–12, the relative/absolute target choice at L57–66, the resolves-not-merely-links proof at L114–120) · the retired v1 plugin's `hooks/plant-link-pretooluse.sh` (whole file, 62 lines; why a third planting site exists at L5–15) · the three planting sites — the retired v1 plugin's `hooks/install.sh` L26, the retired v1 plugin's `hooks/create-worktree.sh` L160, the retired v1 plugin's `hooks.json` L25–34 · [`cli-interface-convention.md`](../claude-plugins/fabrika/docs/cli-interface-convention.md) rule 5 L185–193 and the exit-code table L97–105 · [`skill-conventions.md`](../claude-plugins/fabrika/docs/skill-conventions.md) §4 L100–123 · [`../claude-plugins/fabrika/skills/check-epic-plan/SKILL.md`](../claude-plugins/fabrika/skills/check-epic-plan/SKILL.md) L152–156 · the live fabrika skill corpus under [`../claude-plugins/fabrika/skills/`](../claude-plugins/fabrika/skills/). Line numbers true at commit `a2729e6e`.
...
**Graded against:** `packages/pipeline-cli/src/tools/spawn-guard/command.ts` (deleted with the tool — ADR [0282](../.decisions/0282-spawn-guard-retired.md); readable at the pinned commit) L19–29 (the mode's own header, including why the probe imports nothing), L129–141 (the `createRequire` probe), L143–156 (the signal text), L158–177 (the command — `additionalContext` at L164–168, the exit-2 stderr note at L169–173) · the retired v1 plugin's `hooks.json` L15 and [`../.claude/settings.json`](../.claude/settings.json) L87 (the only two wirings in the repo, both through the wrapper) · the retired v1 plugin's `hooks/guard.sh` L25–34 (the fail-open invariant), L51–55 (the CLI-absent no-dispatch), L57–75 (the marker gate), L79 (the dispatch) · the retired v1 plugin's `hooks/install.sh` L69–73 and L77 (the marker is written only after a verified install and dropped on any failure) · [`../packages/fabrika-cli/README.md`](../packages/fabrika-cli/README.md) *How it is delivered* L44–52 (the five-row table), L57 (the silencer), L74–80 (`FABRIKA_DEBUG`), L91–98 (the unpublished-global callout naming exit `127`) · [`cli-interface-convention.md`](../claude-plugins/fabrika/docs/cli-interface-convention.md) rule 3 L97–108 (the exit-code table), rule 5 L185–198 (the literal is `fabrika`), rule 6 L239–242 · [`../packages/fabrika-cli/src/delegate/resolve.ts`](../packages/fabrika-cli/src/delegate/resolve.ts) L82–89 (the foreign-checkout refusal) and L132–139 (`globalWarning`) · [`../packages/fabrika-cli/src/delegate/entry.ts`](../packages/fabrika-cli/src/delegate/entry.ts) L77–96 (the branch dispatch) · [`../claude-plugins/fabrika/.claude-plugin/plugin.json`](../claude-plugins/fabrika/.claude-plugin/plugin.json) (no `hooks` key). Line numbers true at commit `5226c492`. The README citation is historical from here: the *unpublished-global callout* it names was rewritten in #6543 and now states the package is on the registry, with `127` reached only on a machine that has not installed it — the substance this record leans on (`127` = the verb never ran) survives the rewrite, the label does not. ADR [0250](../.decisions/0250-fabrika-hook-cannot-run-fails-open.md)'s 2026-08-20 amendment carries the same correction.
...
- **Whether fabrika declares a hook surface at all** was [#5074](https://github.com/kamp-us/phoenix/issues/5074)'s, and it now has: see [The surface](../claude-plugins/fabrika/docs/hook-surface.md#the-surface). This record answers only that `freshness` needs nothing on it, which the landed surface does not change — the hook declared there is the surface's own proof, not a degraded-state signal.
...
**Verdict:** PORT — **superseded 2026-08-20 by ADR [0331](../.decisions/0331-fabrika-spawn-hook-retired.md): the port is deleted and the hook is gone.** Everything below is the record as graded; it is history now, not a description of live code. The founder's re-ruling: model discipline is a per-run human choice, a stale allowlist denying a new model generation is the guard's proven failure mode, and the harnesses the fleet also runs on never fire the hook at all. The one artifact that survives is [`../packages/fabrika-cli/src/models.ts`](https://github.com/kamp-us/phoenix/blob/16d3d2d12d686b01670f9bd6e88a1d66f4f4afc0/packages/fabrika-cli/src/models.ts), now vocabulary rather than a gate.

**Retirement note, 2026-09-10.** ADR
[0374](../.decisions/0374-retire-unused-model-vocabulary.md) deletes that remaining table and its
tests. The survival and consumer claims in this graded passage are historical. The eval work
tracked in [#5158](https://github.com/kamp-us/phoenix/issues/5158) is closed and no longer calls
the table. Model-table links in this passage point to the earlier source in git.

**Graded against:** `packages/pipeline-cli/src/tools/spawn-guard/spawn-guard.ts` (deleted with the tool — ADR [0282](../.decisions/0282-spawn-guard-retired.md); readable at the pinned commit) L31 (`ALLOWLIST`), L43 (`MODEL_ALIASES`), L54 (`DEFAULT_PIN`), L87 (the canonicalization), L92–L145 (`decideSpawn`) · `packages/pipeline-cli/src/tools/spawn-guard/command.ts` (likewise deleted) L54–L114 (the `guard` mode: the stdin read, the `WORKFLOW_MODEL` read, the three renderings) · the retired v1 plugin's `hooks.json` (the v1 `PreToolUse` declaration) · [`../packages/fabrika-cli/src/delegate/resolve.ts`](../packages/fabrika-cli/src/delegate/resolve.ts) L66–L91 · [`cli-interface-convention.md`](../claude-plugins/fabrika/docs/cli-interface-convention.md) rule 5 L185–198, rule 6 L239–242 · ADR [0092](../.decisions/0092-gates-fail-closed-on-zero-scope.md), ADR [0116](../.decisions/0116-spawn-guard-durable-default-pin.md), ADR [0238](../.decisions/0238-fabrika-reimplements-v1-never-calls-it.md). Line numbers true at commit `066b9c4a`.
...
**Not graded here:** the fail-open-versus-fail-closed dispatch policy, ruled at [#5079](https://github.com/kamp-us/phoenix/issues/5079) and recorded on [the surface page](../claude-plugins/fabrika/docs/hook-surface.md#the-dispatch-failure-policy-point).
...
**Graded against:** the retired v1 plugin's `hooks/worktree-sweep-detach.sh` (whole file, 54 lines; L31–33 the no-data-dir fail-open, L41–50 the previous-run report, L52 the background dispatch) · the retired v1 plugin's `hooks/resolve-data-dir.sh` L20–28 · `../../../packages/pipeline-cli/src/tools/worktree-reap/worktree-reap.ts` L17–34 (the two signals, and signal 2 having no producer), L67 (reading the harness's own lock stamp) · [`../.claude/settings.json`](../.claude/settings.json) L78 · Claude Code 2.1.233, observed. Line numbers true at commit `08793089`.
