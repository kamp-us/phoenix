...
| `sources.ts` | `fateSource<Row>({...})` executor + hand-written `AnySourceDefinition` literal | one `Fate.source(ViewClass, {id}, handlers)` per entity; the `SourceExecutor`/`AnySourceDefinition` annotations and casts disappear. A **synthetic** entity with no fetch path keeps a capability-less registration via `Fate.syntheticSource(ViewClass)` (pasaport's `Contribution`); a **root-only** synthetic entity stays sourceless via a string-typed query (stats' `LandingStats`) — see [fate-effect-sources.md](./fate-effect-sources.md) "The escape hatch" |
