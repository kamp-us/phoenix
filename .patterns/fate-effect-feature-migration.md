...
|---|---|---|
| `errors.ts` | plain `Schema.TaggedErrorClass`es, wire codes in the central `WIRE_CODE_BY_TAG` registry | same classes + `{[fateWireCode]: "<CODE>"}` third parameter — codes copied **verbatim** from the registry row; an `errors.unit.test.ts` enumeration pin per pair |
