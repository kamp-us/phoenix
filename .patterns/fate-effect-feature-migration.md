...
| `sources.ts` | `fateSource<Row>({...})` executor + hand-written `AnySourceDefinition` literal | one `Fate.source(ViewClass, {id}, handlers)` per entity; the `SourceExecutor`/`AnySourceDefinition` annotations and casts disappear |
