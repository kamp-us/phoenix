...
## An entity frame merges whole, so a viewer-derived field cannot ride one {#entity-frames-merge-whole}

`live.update(type, id, {changed, data})` reads as "tell subscribers these fields moved", and it is not that. Two facts, both verified against `@nkzw/fate@1.3.1` and phoenix's own publisher:

- **`changed` is inert in phoenix.** Stock fate uses it server-side (`server/http.ts` narrows each subscriber's selection with it, and the narrowed selection rides back to the client as the frame's `select`), but phoenix serves the SSE protocol from `LiveDO` and never runs that drain path — its frame builder is `entityFrame` in [`live-publisher.ts`](../apps/web/worker/features/fate-live/live-publisher.ts), which emits `{data}` and drops `changed` on the floor. Every `changed: [...]` in a mutation today is documentation, not behaviour.
- **The client merges the whole payload.** With no `select` on the frame, fate's store does `{...previous, ...partial}` — every key present on `data` overwrites that subscriber's cached value, and the operation's own selection does not mask it. A key the payload *omits* is preserved (the merge only copies keys it has).

So the published node is not "the changed fields", it is **one viewer's whole answer broadcast to every subscriber of that entity**. Any field resolved against the mutator — `sandboxedInPlace` off the request's `SandboxViewer`, and the viewer scalars `myVote` / `isSaved` — reaches everyone else as *the mutator's* value, in both directions: it leaks a `true` to viewers holding no entitlement, and it erases a `true` a subscriber correctly read.

The fix for a viewer-derived field is to **leave the key off the payload**, not to stamp a safe `false` — `false` is still a broadcast overwrite. Pano and sözlük route every entity `update` through `viewerBlindUpdate` ([`kunye/sandbox.ts`](../apps/web/worker/features/kunye/sandbox.ts)) so the çaylak marker is dropped at the seam rather than at each call site (#6462). The viewer scalars still ride, and [#2213](https://github.com/kamp-us/phoenix/issues/2213) is the reason they do — the shape was widened to carry *real* values rather than narrowed to carry none.
...
The reachable instance is the çaylak→yazar promotion: one batch clears `sandboxed_at` across the author's whole backlog ([`Pasaport.ts`](../apps/web/worker/features/pasaport/Pasaport.ts)), so every opted-in yazar holding those rows holds a `sandboxedInPlace: true` that is now false. The seam is an **invalidation**, not a payload, and it goes out at both subscription shapes — `publishPromotion` ([`promote-live.ts`](../apps/web/worker/features/pasaport/promote-live.ts)) invalidates the `posts` feed, each swept comment's `Post.comments` thread and each swept definition's `Term.definitions` topic, and then each swept `Post` / `Comment` / `Definition` row by id. fate answers a connection invalidation by dropping the connection and calling `loadConnection` again; an entity one it will answer by re-`fetchById`ing the row, once the `@nkzw/fate` pin moves — today it dispatches that frame nowhere (see below). Either re-read runs the normal read path, so every viewer-derived field is re-derived against the subscriber's *own* viewer.

Both halves are needed, and neither substitutes for the other: a row a client holds by id sits in no connection, so no topic invalidation reaches it, and a connection re-read repairs rows the sweep never names individually.

The topics and the ids have to be captured **before** the batch: afterwards `sandboxed_at` is null and a swept row is indistinguishable from one the author already had live. That is why [`SandboxSweep`](../apps/web/worker/features/pasaport/sandbox-sweep.ts) carries both, read out of the same three pre-batch queries.

**The publish side of the entity half is complete; the read side is not yet.** `EntityFrame` in [`protocol.ts`](../apps/web/worker/features/fate-live/protocol.ts) carries a `{type: "invalidate", id}` variant beside `{delete: true, id}` and `{data, select?}`, `live.invalidate(type, id)` emits it, and the `LiveDO` relay fans it on the same `kind: "next"` path as every other entity frame ([ADR 0314](../.decisions/0314-entity-invalidate-frame-upstream.md)). fate's entity dispatch has no invalidate arm, so until [#6661](https://github.com/kamp-us/phoenix/issues/6661) ships upstream and the `@nkzw/fate` pin moves, a published entity invalidation arrives at a client with nothing to dispatch it into. The live instance is the post detail page: `PostDetailView` in [`PanoPostDetail.tsx`](../apps/web/src/pages/PanoPostDetail.tsx) spreads `PanoPostHeaderView`, which selects `sandboxedInPlace`, over a `Post` read by slug — it stays stale, and [#6565](https://github.com/kamp-us/phoenix/issues/6565) stays open, until the pin moves.
