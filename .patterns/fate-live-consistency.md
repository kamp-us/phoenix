...
**That seam reaches connection subscribers only.** An invalidation names a connection, so it repairs a row a client is holding *through* one — a feed, a comment thread, a term's definitions. A client holding the same row through an **entity** subscription re-reads nothing and keeps its stale marker until something else makes it read again. The live instance is the post detail page: `PostDetailView` in [`PanoPostDetail.tsx`](../apps/web/src/pages/PanoPostDetail.tsx) spreads `PanoPostHeaderView`, which selects `sandboxedInPlace`, over a `Post` read by slug. There is no wire frame that would fix it — `EntityFrame` in [`protocol.ts`](../apps/web/worker/features/fate-live/protocol.ts) is `{delete: true, id}` or `{data, select?}`, with no entity-level invalidate — so closing it means a new frame and a `LiveDO` relay path for it ([#6565](https://github.com/kamp-us/phoenix/issues/6565)). Read this section as the rule for connection subscribers, not as a claim that no subscriber is left stale.
