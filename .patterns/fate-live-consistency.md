...
**That seam reaches connection subscribers only.** An invalidation names a connection, so it repairs a row a client is holding *through* one — a feed, a comment thread, a term's definitions. A client holding the same row through an **entity** subscription re-reads nothing and keeps its stale marker until something else makes it read again. The live instance is the post detail page: `PostDetailView` in [`PanoPostDetail.tsx`](../apps/web/src/pages/PanoPostDetail.tsx) spreads `PanoPostHeaderView`, which selects `sandboxedInPlace`, over a `Post` read by slug. The publish side of the fix now exists: `EntityFrame` in [`protocol.ts`](../apps/web/worker/features/fate-live/protocol.ts) carries a `{type: "invalidate", id}` variant beside `{delete: true, id}` and `{data, select?}`, `live.invalidate(type, id)` emits it, and the `LiveDO` relay fans it on the same `kind: "next"` path as every other entity frame ([ADR 0314](../.decisions/0314-entity-invalidate-frame-upstream.md)). **The read side does not.** fate's entity dispatch has no invalidate arm, so until [#6661](https://github.com/kamp-us/phoenix/issues/6661) ships upstream and the `@nkzw/fate` pin moves, a published entity invalidation arrives at a client with nothing to dispatch it into — the detail page stays stale, and [#6565](https://github.com/kamp-us/phoenix/issues/6565) stays open. Read this section as the rule for connection subscribers, not as a claim that no subscriber is left stale.
