...
- **Read.** A read may sit wherever it is convenient, including directly in a route handler — it does **not** have to go through a feature service. Reach for raw SQL on a read when the query is genuinely better expressed that way.
...
- **adopt** — for a rename whose SQL is byte-identical (proven against the state's last-deploy hash map): use **Run workflow** on [deploy.yml](../.github/workflows/deploy.yml), select `main` and set `migrations_drift` to `adopt`. This sets `D1_MIGRATIONS_DRIFT=adopt` on that deploy step (`worker/env.ts` → `migrationsDriftStrategy` on `PhoenixDb`) to re-key the record in `drizzle_migrations` without re-running the SQL. The input defaults to `refuse`; dispatches outside `main` fail. One-shot operator consent, never a standing setting.
