...
Add an eighth crew message kind, `Release`, carrying `ReleaseClaim {resource, claimant: PeerId, at: Timestamp}`, defined in `protocol/schema.ts` + `protocol/group.ts` as `Rpc.make("Release", { payload: Messages.ReleaseClaim })`. It is **fire-and-forget** — no `success` schema, so it defaults to `Schema.Void`, the documented fire-and-forget idiom already used for `AnnouncePresence` / `Heartbeat` (`protocol/group.ts` docblock; grounded in effect-smol's cluster-entities example, `ai-docs/src/80_cluster/10_entities.ts` L12–19, where an `Rpc.make` with no `success` is the void-reply kind). Releasing is idempotent: releasing a claim you do not hold is a no-op, so no reply is needed.
