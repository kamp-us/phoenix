...
> Amendment 2026-08-30: The founder [ruled for #7306](https://github.com/kamp-us/phoenix/pull/7351#issuecomment-5470229879) that CODEOWNERS must hold the path-complete `review-ui` evidence authority: `packages/fabrika-cli/src/review-ui/` plus the exact delegated files `src/ship/github.ts`, `src/capture/capture.ts`, `src/capture/png.ts`, `src/ui/png.ts`, and `src/capture/upload.ts`. Those paths decide the governed GitHub revision, capture bytes and browser evidence, PNG validity, and verified upload that may satisfy the merge-blocking namespace. This qualifies 0299's preserved coverage statement without widening ownership to any other `fabrika-cli` path.
