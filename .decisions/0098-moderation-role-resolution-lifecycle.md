...
Add a moderator capability to `user` as a **server-managed field** following the `user.type` / `input:false username` precedent: a `role` column (closed set, e.g. `["member","moderator"]`, default `member`) — or equivalently a boolean `is_moderator` — set only by a server-side path (an offline D1 grant script and/or a karma/künye-gated promotion later), **never** by public input. It is declared to better-auth with `input:false` so no client write can reach it. It is *not* carried in the session's `CurrentUserInfo` (which stays the minimal id/email/name/image — `CurrentUser.ts:31`); per the CLAUDE.md rule, richer reads go through a domain service. Authority is read from D1 at the point of use, not trusted from session state.
...
- **Migration cost:** add `role` (or `is_moderator`) to `user` with `input:false`; add `resolver_id` + `resolved_at` + `resolution` to `content_report`; widen `status` to the closed state set. The reverse index already exists. No new index for the repeat-offender signal.
