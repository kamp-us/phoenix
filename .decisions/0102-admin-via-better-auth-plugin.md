...
> Amended by [ADR 0107](0107-capability-authz-framework.md): the admin-capability *authorization* mechanism (a better-auth admin-plugin role/AC model on the worker) is superseded — admin becomes a `Relation`-backed `Admin` capability in the capability-as-Effect framework, and better-auth is authn-only (no admin/AC plugin used for authorization). The CLI-as-client surface and the fail-closed, one-place-server-capability intent stand; the better-auth-plugin *authorization* substrate does not.
