...
human code-owner's approval to merge, wherever that code lives. It just moved into
`packages/fabrika-cli/src/ci/`, so the fence moves there too — that one directory, and no more of
fabrika-cli.

## Context

The `ci-required` verdict core is the pass/fail logic behind the single always-on required status
context on `main`. It used to live in `packages/pipeline-cli/src/tools/ci-required/`, which ADR
...
[#5720](https://github.com/kamp-us/phoenix/issues/5720) moved it to
`packages/fabrika-cli/src/ci/required.ts` + `required-bin.ts`, and rewired `ci.yml`'s `ci-required`
job onto the new bin. The pipeline-cli copy stayed behind as the frozen v1 baseline nothing runs
(ADR [0238](0238-fabrika-reimplements-v1-never-calls-it.md)).

`CONTROL_PLANE_RE` did not move with it. So the covered path went dead and the live path went
uncovered, which is not a cosmetic mismatch: `.github/CODEOWNERS` carries no `*` catch-all, and the
`main` ruleset pairs `required_approving_review_count: 0` with `require_code_owner_review: true`
(ADR [0135](0135-hard-gate-control-plane-team-codeowners-approve-then-enqueue.md)). A path matching
no row therefore merges at **zero** approvals. A pull request touching only
`packages/fabrika-cli/src/ci/` would have landed the merge-deciding verdict logic unreviewed.
...
**The founder ruled it on [#6164](https://github.com/kamp-us/phoenix/issues/6164), 2026-08-18** —
extend the fence, scoped to the CI checks. Verbatim: "i'm ok with it as long as it's scoped to the
ci checks, if not i trust fabrika enough to self drive itself."
...
same shape to the package the core moved into. The test it applies is ADR
...
The unit of coverage is the **directory**, not the file. `required.ts` decides the verdict,
`required-bin.ts` is what the job runs, and a test that pins either one is as gate-critical as the
logic it pins — an unreviewed edit that guts the test guts the guard. A file-level enumeration would
also rot as the core grows, which is the failure ADR 0218's non-recursive `src/[^/]+$` pattern was
written to avoid.
...
- Epic #5720 Phase 3 may delete the pipeline-cli `ci-required` CODEOWNERS rows only after this
  branch has landed. Deleting the dead-but-covering rows first leaves the live core with no coverage
  at all.
...
- **Epic #5720 Phase 3 is unblocked.** The live core is covered, so dropping the pipeline-cli rows
  no longer strips the fence.
