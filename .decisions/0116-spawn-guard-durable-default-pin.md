...
> Amendment 2026-08-20: fabrika's reimplementation is retired too — ADR [0331](0331-fabrika-spawn-hook-retired.md) deletes `fabrika hook spawn`, its decision and its `PreToolUse` declaration, which is why the `hook/spawn.ts` link above is dropped (read it at the pinned commit). **This decision no longer binds anything.** `DEFAULT_PIN`/`ALLOWLIST`/`canonicalModel` survive in [`packages/fabrika-cli/src/models.ts`](../packages/fabrika-cli/src/models.ts) as vocabulary only — no code resolves a pin or denies a spawn. The whole record is history from here.
