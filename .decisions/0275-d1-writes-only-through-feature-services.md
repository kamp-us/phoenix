...
The write half is a hard boundary. The read half is deliberately not one: a route that only needs rows may take `orDieAccess(yield* Drizzle)` and query.
...
**The obvious guard shape does not work, and this is the useful part to record.** #5473 proposes a fail-closed check that reds on `yield* Drizzle` / `yield* Database` in a non-test file outside `apps/web/worker/db/`. Of the sites counted at the original HEAD, **17 remain**, and it is wrong about nearly all of them:

- **15** are the feature services this ADR exists to permit — `vote/Vote.ts`, `reaction/Reaction.ts`, `kunye/VouchLedger.ts`, `kunye/RelationStore.ts`, `mute/Mute.ts`, `pasaport/Pasaport.ts`, `pasaport/email-delivery-log.ts`, `search/Search.ts`, `pano/Pano.ts`, `pano/Bookmark.ts`, `report/Report.ts`, `sozluk/Sozluk.ts`, `bildirim/Notification.ts`, `funnel/Funnel.ts`, `stats/Stats.ts`.
