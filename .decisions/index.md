...
| [0064](0064-skills-are-control-plane-blocking.md) | `skills/**` is control-plane → BLOCKING (manual human merge) — restores the blocking property #231 silently dropped when it moved skills out of `.claude/skills/`; two independent axes: routing (review-code, unchanged per 0063) vs merge-authority (now blocking); safe-by-default stopgap until a dedicated `review-skill` gate (#371); amends 0063 (routing kept, merge-authority corrected) and extends 0053's control-plane boundary from "by path" toward "by nature" | accepted | 2026-06-15 |
