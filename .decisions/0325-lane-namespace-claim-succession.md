...
## Amendment — 2026-09-15 ([#7778](https://github.com/kamp-us/phoenix/issues/7778)): an adopt marker is always reachable, and it fences only what it postdates
...
**An adopt fences only a claim marker posted after it.** The fence that keeps one succession from
answering `Mine` to two drivers read every adopt over the winning marker's session, whatever their
order. A succession adopts a claim that already stands, so an adopt *older* than the winning marker
adopted some earlier claim and says nothing about this one. Ordering is GitHub's `created_at` with
