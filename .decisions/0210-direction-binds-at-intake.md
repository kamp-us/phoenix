...
**Relationship to ADR [0072](0072-milestones-encode-strategic-sequencing.md) §4 — 0072 §4 outranks this ADR's park lane; the clock never reaches a parked-by-design cluster.** 0072 §4 makes milestone-absence itself the durable parked signal ("Absence is meaningful — don't invent homes for frozen work"). The two rules key off **orthogonal dimensions** — 0072 §4's signal is milestone-*absence*, this ADR's park lane is a *label* — so one issue can carry both, and the live board already holds several that do. ADR 0208 (open lane, PR #3896) goes further still, relocating freeze-by-absence's signal *into* the `wayfinder:backlog` label as a permanent, milestone-less workstream. The sets therefore intersect **by design**, so the reconciliation is a **precedence rule, not a disjointness claim**: **a cluster parked by design — deliberately unmilestoned per 0072 §4, or a standing lane per 0208 — is neither clocked nor killed, whatever labels it carries.** That precedence is tested first and outranks both the 3-week expiry and 0202's kill verdict, which is what keeps the deliberately-unmilestoned new products (imge, kampus-CLI, künye) frozen rather than expired or closed. Precedence leaves 0072's own rule intact, so 0072 is **unamended** and its status line is untouched.
...
- Fog-parked work expires at the founder-set window (3 weeks), amending ADR [0203](0203-fog-reports-route-to-wayfinder-backlog.md) in part; a parked-by-design cluster (0072 §4, or an 0208 standing lane) is exempt from both the clock and 0202's kill verdict, and that exemption is tested first.
...
- Cross-references the standing-lane exemption of ADR 0208 (open lane, PR #3896) — both as the precedent shape for a label-keyed bound and as the second carrier of the parked-by-design exemption above.
