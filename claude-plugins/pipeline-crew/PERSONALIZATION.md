...
| — engine WIP cap | `roles.engineering-manager.wipCap.{productLanes, platformLanes}` | A conductor engine's bounded concurrent-lane count, lane-partitioned — how many product vs platform/pipeline coders one engine drives at once before queueing the rest. The overall cap is the split's sum; the borrow/rebalance behavior is doctrine shipped in the engineering-manager def (the seam carries only the per-install values). Both counts are required and must be integers >= 0 — the launcher decodes them and delivers them in each engine's boot turn, so a missing or malformed cap fails the launch closed rather than leaving the engine to improvise (#4330). | `<wip-cap-product-lanes>`, `<wip-cap-platform-lanes>` |
