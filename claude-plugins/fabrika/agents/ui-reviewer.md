...
`review-ui` skill by name, with that skill already in context. The shell names the actor and never
the skill it loads, so the `ui-reviewer` shell runs the `review-ui` skill. Every step, rubric and
