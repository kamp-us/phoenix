...
evidence lives — the portable replacement for v1's hardcoded knowledge of one repo's dev server:
...
`harness` is the path `designHarness` declares, and the `design-harness.json` above is the shipped
default for that key, not a fixed name. `lawSource` is `"registry"` when the registry file exists,
...
1. **Store tier** — when the harness config declares `evidenceStore`: `PUT` each PNG
   content-addressed (`<store>/<sha256>.png`, the same idiom the golden store uses), then `GET`
