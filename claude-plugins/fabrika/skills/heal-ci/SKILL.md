...
read access to the base branch's protection surface that token happens to carry — the skill
degrades explicitly when it carries none rather than assuming it has it. Writes used: one workflow
...
| The base ref's declared required-status contexts (branch protection or a ruleset) | `surface` compares them against the runs that actually post — the whole check-surface class | **degrade**, on two absences this run must never conflate. A base with **no** declared requirements answers `no-requirements` at exit `0` — a proven fact about a repo that gates nothing. A protection surface present but **unreadable at this token's permission** answers `unprobeable`: the check-surface axis is skipped, stated, and never reported as clean. |
