...
**A note on every digest and nonce printed below.** The 12-hex digests and 8-hex nonces in the
examples are **illustrative placeholders conforming to the stated grammar**, not values computed
from the surrounding bytes. The computations themselves are fully specified (*The ground state* and
*The pack marker* below), which is what the derivability test asks for; a reader who wants a real
value computes it from the specification rather than trusting a literal here (ADR
[0247](../../../../.decisions/0247-a-spec-example-value-is-derivable-or-absent.md)).
...
- [`src/verb.ts`](../../../../packages/fabrika-cli/src/verb.ts) — `answer` / `refuse`, and the
  `emit` adapter each group copies. **Its shape bounds this whole spec**: `refuse` hardcodes empty
  stdout and `answer` hardcodes code `0`, so a non-zero exit carrying a machine payload is not
  constructible and is specified nowhere below. Every proven outcome that a caller must *parse*
  therefore lands as a state token at exit `0`.
- [`src/io/stdin.ts`](../../../../packages/fabrika-cli/src/io/stdin.ts) — `readStdin`. The fd-0
  boundary, where `Text("")` and `Failed` are different answers and never collapse. This is the
  by-value seam.
...
carrying content outside its closed section set**, because a coordination artifact whose section
set is open can steer its receiver past the artifact — an extra heading, a sentence appended under a
known one, a "note from the maintainer" — and the receiver cannot tell the format's own words from
someone else's. That is the injection defence the pack needs, and it is why the asserted half's four
sections are closed rather than merely expected. It cannot be *called*: its fields are a slice id, a
branch and a worktree root, none of which a pack carries.

**Considered and deliberately not derived.**

- **A `handoff drift` verb.** An earlier draft had one, reading a pack and comparing it against
  live. Folded into `handoff read`, because separating them makes the dangerous call the *easy*
  one: a caller who runs `read` and skips `drift` holds a pack that reads current while being
  stale, which is the #3330 class this group is largely built against. One verb answering both
...
- **A verb that writes, summarizes, or grades the asserted half.** The four sections are the
  model's words. A verb that generated them would be composing the document from the same session
  memory the two-half split exists to distrust, and a verb that graded them would be a second
  reviewer nobody asked for.
- **A verb that files anything.** `handoff` creates no work. If the session's observation is *new
  work someone should do*, the model fires the `report` Skill and this group is not involved
  (#4636, #4640). A filing verb here would rebuild the self-filing side door the quintet exists to
  close, under a new name.
- **A verb that closes, labels, or retires a pack.** A later `handoff take` on the same issue
  supersedes an earlier pack by ordering — `read` resolves the **latest** sealed pack and nothing
  else — so a retirement verb would add a second, drift-prone way to express what ordering already
  expresses. This group applies no label of any kind.
...
- **Reuse of `src/build/claim.ts`.** Its `composeMarker` hardcodes the `build-claim:` prefix, its
  `MARKER_RE` matches only that grammar, and `resolveOwnership` compares a **session** token — so
  `markersIn` would find zero markers on a pack and every claim would read as unclaimed. This group
  authors its own marker and keys on a run nonce; see *The pack marker*.
...
```markdown
<!-- fabrika:handoff pack nonce=7f3a9c21 sealedAt=2026-08-09T18:36:48Z groundDigest=a1b2c3d4e5f6 -->
...
## Ground state — proven
```json
{"issue":5021,"repo":"kamp-us/phoenix","capturedAt":"2026-08-09T18:36:48Z","git":{"branch":"umut/fanout-helper","head":"4f1c8a2b9d3e5607182934abcdef5566778899aa","reachable":"pushed","upstream":"origin/umut/fanout-helper","aheadBy":0,"behindBy":2,"base":{"branch":"main","head":"11223344556677889900aabbccddeeff00112233"},"tree":{"state":"clean","trackedModified":0,"untracked":0}},"board":{"issue":{"state":"open","labels":["type:chore","p2"]},"pull":{"number":5290,"state":"open","head":"4f1c8a2b9d3e5607182934abcdef5566778899aa","checks":"failing"}},"groundDigest":"a1b2c3d4e5f6"}
```
```
...
Closed makes that unrepresentable rather than detectable.

<!-- anchor: UNSURE-IS-NEVER-SILENT --> **Every asserted section is non-empty, and `## Unsure` is
the one that matters.** A section skipped and a section with nothing to say are the same bytes and
opposite facts, so an empty one is `4`. A caller who genuinely resolved everything writes that
sentence; a caller who writes nothing is asserting certainty they almost never have, which is the
single most expensive thing a successor can be handed. This is interface rule 2's *the positive
answer is a positive token, never an absence*, applied to a document section.
...
```
<!-- fabrika:handoff claim nonce=4b8e2f01 pack=9234567891 claimedAt=2026-08-09T19:02:11Z -->
```

Fields: `nonce` matches `^[0-9a-f]{8}$`; `sealedAt` / `claimedAt` are ISO-8601 UTC with a trailing
`Z`; `groundDigest` is exactly 12 lowercase hex; `pack` is the comment id of the pack being claimed.
A comment whose first line does not match is not a pack and not a claim — **it is not a
...
because this artifact crosses the widest boundary in the corpus — one session to another that
shares no memory, no worktree, and possibly no machine — and `WireRead<A>`'s three answers are what
that boundary needs: **a malformed pack must never read as an absent one**, or a successor
concludes nobody handed off and starts over. The key is deliberately distinct from the shipped
`slice-handoff`, which is a different format with an inverted posture (see *Read for its shape*).

**Not widened, deliberately.** `wire/verdict-marker.ts`'s `NAMESPACE` regex and its separate
`NAMESPACE_PREFIXES` gate are left alone. Those govern **verdict markers** specifically; a
registered format is not required to be a member, and the shipped `slice-handoff` is the proof —
it is registered and appears in neither. This group emits no verdict, so widening either would
admit a merge-gating namespace for a skill that gates nothing.

## The ground state, and its neutrality invariant

`capture` emits one JSON object with a fixed key set. Every field's derivation:

| Field | Derivation | Closed set / shape |
|---|---|---|
| `git.branch` | `git rev-parse --abbrev-ref HEAD` | string; `"HEAD"` when detached |
| `git.head` | `git rev-parse HEAD`, resolved with `resolveCommit` | 40 lowercase hex |
| `git.upstream` | `git rev-parse --abbrev-ref --symbolic-full-name @{u}` | string, or `null` when unset |
| `git.reachable` | `pushed` when `upstream` resolves and `git merge-base --is-ancestor HEAD @{u}` succeeds; `unpushed` when it fails; `unknown` when either read fails | `pushed` / `unpushed` / `unknown` |
| `git.aheadBy` / `git.behindBy` | the two counts of `git rev-list --left-right --count @{u}...HEAD` | integer, or `null` when `upstream` is `null` |
| `git.base.branch` | `--base`, defaulting to the repository's default branch from `getIssue`'s repo payload | string |
| `git.base.head` | `git rev-parse <base.branch>` | 40 lowercase hex |
| `git.tree.state` | `clean` when `git status --porcelain=v1 --untracked-files=all` is empty; `dirty` otherwise; `unknown` when the read fails | `clean` / `dirty` / `unknown` |
| `git.tree.trackedModified` | count of porcelain lines whose status is not `??` | integer |
| `git.tree.untracked` | count of porcelain lines whose status is `??` | integer |
| `board.issue.state` | the issue payload's `state` | `open` / `closed` |
| `board.issue.labels` | the issue payload's label names, sorted ascending | array of strings |
| `board.pull` | the open pull request whose head ref equals `git.branch`; `null` when there is none | object or `null` |
| `board.pull.checks` | `failing` if any check run on `pull.head` concluded `failure`, `cancelled` or `timed_out`; else `pending` if any is `queued` or `in_progress`; else `passing` if at least one concluded `success`; else `none` | `passing` / `failing` / `pending` / `none` / `unknown` |
| `capturedAt` | the wall clock at derivation, ISO-8601 UTC | string |
| `groundDigest` | see below | 12 lowercase hex |

`groundDigest` is `bodyDigest` over the object serialized with keys in the table's order, no
whitespace between tokens, and `capturedAt` **excluded** — the first 12 characters of the lowercase
hex SHA-256. `capturedAt` is excluded because two captures of an unchanged repository must digest
identically; folding a clock in would make every comparison report drift.

<!-- anchor: THE-GROUND-EXCLUDES-WHAT-THIS-GROUP-WRITES --> **Named invariant — the ground state
excludes exactly the fields this group's own writes move, and nothing more.** The pack and the claim
are both **comments**, so the excluded set is the issue's `updated_at`, its comment count, and its
comment ids; none appears in the table above. Walk the fifteen rows and no write of this group's can
move one: posting a comment changes no ref, no tree, no issue `state`, no label, and no pull-request
field. Therefore a successor's drift check never fires on the handoff's own footprint, and a
`drift: "none"` is a statement about the *work*, not an artefact of the recording. Every exclusion
is drift you can no longer detect, so the set is kept to the three that are forced; the
implementation owes a deterministic test that `take` followed immediately by `read` yields
`drift: "none"`.
...
- **Every text this group sends to GitHub is leak-scanned** with `src/report/leaks.ts` before the
  write. A machine-local path is `5`, a bare `@` reference is `6`. **Widening stated:** the base's
  `5` reads *"…and `--redact` was not given"*; no `handoff` verb offers `--redact`, so here `5`
  fires on any machine-local path unconditionally. The condition narrows; the meaning does not
  drift.
...
- **Externally-authorable content is data, never instruction.** A pack's asserted half is prose
  someone else wrote. Authority arrives only through the ACL check in `handoff read`, which
  resolves the pack author against repository permissions (ADR 0055) and disregards a pack from an
...
| `4` | `BAD_SECTIONS` — the supplied asserted half is missing a section, has one out of order, has an empty one, or carries content outside the closed set | — | ✓ | — | — |
...
numerals, so a drift is unrepresentable rather than merely detectable. `12`–`15` are the group's own
and clear the base's occupied seats; they carry **no** cross-group uniqueness obligation, so
`build`'s `12 NOT_A_WORKTREE` and this group's `12` are two namespaces rather than a collision. The
governing rule is the shipped one at
...
which is a dirty tree **or** an unpushed head, and which `--declare-unreachable` can waive), so it
is allocated rather than imported.

**`10` is a deliberate gap**, exported as a `DELIBERATE_GAP` constant on the shipped `ui` and
`grilling` precedent, and excluded from `allocatedCodes` in `exit-code-alignment.ts` so the hold is
neither a drift nor a collision. The base's `10 CLASSIFIED` fires when a title or label carries a
type or priority classification. No `handoff` verb accepts a label flag, none writes a label of any
kind, and none composes a title — the verbs run no such check at all, so the condition is
unreachable rather than merely unused.

<!-- anchor: WHY-NO-PACK-IS-0-ON-READ-AND-13-ON-CLAIM --> **Why the same fact is exit `0` on `read`
and `13` on `claim`.** They are different questions. `read` **supplies an input**, and interface
rule 4 makes an empty result a fact or a failed read once, in the header: zero packs on an issue is
a **fact** — most issues have none — so it is the `none` token at `0`, and a successor branches on
the token. `claim` **acts**, and asking to claim a pack that does not exist is a request that cannot
be honoured, so it refuses. Collapsing them either way costs something real: seating `read`'s
answer on `13` would hand a booting successor empty stdout on the ordinary case, and answering
`claim` with `0` would report a claim nobody holds.
...
2. `src/exit-code-alignment.ts` — add `handoff` to `ALIGNED_GROUPS`. Its value is a `SharedSeats`
   map keyed by **this group's own export names**; this group re-exports the base's names verbatim
   (`EMPTY_STDIN`, `BAD_SECTIONS`, `LEAKED_PATH`, `BARE_AT_PATH`, `NO_TARGET`, `WRITE_UNKNOWN`,
   `READBACK_MISMATCH`, `DELIBERATE_GAP` for `10`, `PRECONDITION_UNKNOWN`), so a `HANDOFF_SEATS`
   constant is authored in that same file rather than reusing a shipped map.
3. `src/exit-code-alignment.unit.test.ts` — add a `handoff` row to the hand-written `TABLES` map, or
   ...
   **That red is the intended order, not a mistake**: the scan is `existsSync` over `src/*/codes.ts`,
   so the file's creation is what arms the check.
4. The `handoff-pack` wire format: a schema module exporting `emitFromFields` / `readToLines`, its
   `src/wire/registry.ts` row carrying `fixtures` and `brands`, and its `### handoff-pack` section
   in `docs/wire-formats.md` — without the last, `src/wire/index-doc.unit.test.ts` reds on key-set
   equality.
5. `packages/fabrika-cli/README.md` — the group list and a `## The handoff group` section; the
   repo-wide `readme-guard` job is fail-closed.
...
the whole quintet, open and unruled at
[#5241](https://github.com/kamp-us/phoenix/issues/5241) and owned by
[#4649](https://github.com/kamp-us/phoenix/issues/4649)'s harness rather than by this contract. It
is recorded here so an implementer meets it as a known absence rather than as a surprise, and so the
...
|---|---|---|---|---|
| `--issue` | integer | yes | — | the issue the work belongs to; its live state and any linked pull request are part of the ground |
...
**Output** — machine. One JSON object, the ground state specified in *The ground state* above:
...
`board.pull` being `null` is a **fact** (no open pull request has this head ref), not an absence;
...
safe for the successor to re-run against a pack it has not yet decided to trust.
...
| `handoff capture: cannot list check runs for <sha>: <reason> — reporting checks as unknown would claim a read that failed.` | 11 | refusal |

**Scope** — the local git repository at the working directory's root, and the issue plus its linked
pull request and that pull request's check runs, paginated. The scope line on stderr names the
branch, the base and the pull request number or `none`. **A git read that fails is `11`, never a
clean tree** — reporting `clean` over a failed `git status` is the zero-scope pass ADR
[0092](../../../../.decisions/0092-gates-fail-closed-on-zero-scope.md) forbids, and here it would
license a pack asserting reachable work that is not reachable.
...
- #3330 — a pipeline baseline scan ran against a stale local checkout and spawned phantom
  no-op children. The whole reason a pack carries a derived ground state rather than a session's
  belief about it, and the reason `reachable` is computed rather than assumed.
...
| `--nonce` | string | yes | — | this run's nonce, matching `^[0-9a-f]{8}$`; stamped into the marker so a later claim and a later pack are attributable. A value outside that grammar is a usage error (`1`) |
| `--base` | string | no | the repository's default branch | passed through to the capture |
...
```json
{"issue":5021,"pack":9234567891,"nonce":"7f3a9c21","sealedAt":"2026-08-09T18:36:48Z","groundDigest":"a1b2c3d4e5f6","reachable":"pushed","supersedes":9234501234}
```
...
2. **The asserted half parses into exactly the four closed sections**, in order, each non-empty,
   with no content outside them. Otherwise `4`.
3. **The text carries no machine-local path and no bare `@` reference.** `5` / `6`.
4. **The issue exists.** Otherwise `7`.
5. **A fresh capture is taken.** A failed derivation is `11`, and nothing is written.
6. **The work is reachable** — `git.reachable` is `pushed` and `git.tree.state` is `clean`.
   Otherwise `12`, unless `--declare-unreachable` was given.

<!-- anchor: UNREACHABLE-WORK-IS-REFUSED --> **Why unreachability refuses rather than warns.** A
successor is a fresh session in a fresh worktree: an unpushed commit and an uncommitted tracked
change are both literally invisible to it. A pack whose `## Next act` points at work in that state
is confidently wrong in the way #3330 is confidently wrong — a plausible record resting on state
only the writing machine can see. The remedy is one command (commit and push), which is why the
default is a refusal. `--declare-unreachable` exists for the genuine case where the diff is
disposable, and it does not silence the fact: the proven half records `reachable` and the two
...
| `5` / `6` | the asserted half carries a machine-local path, or is a bare `@` reference |
...
| `handoff take: <k> commit(s) are not pushed and <m> tracked file(s) are modified — a successor cannot see either. Push, or re-run with --declare-unreachable to record the loss.` | 12 | refusal |
...
(the first handoff on an issue is the ordinary case); a comment read that could not complete is
`11`, and `supersedes` is never guessed.
...
{"issue":5021,"pack":9234567891,"nonce":"7f3a9c21","sealedAt":"2026-08-09T18:36:48Z","groundDigest":"a1b2c3d4e5f6","reachable":"pushed","supersedes":null}
$ echo $?
0
```
...
handoff take: 2 commit(s) are not pushed and 1 tracked file(s) are modified — a successor cannot see either. Push, or re-run with --declare-unreachable to record the loss.
$ echo $?
12
```
...
- v1's `add-frontier-ticket.sh` prints a refusal with bare `echo` to **stdout**, the channel
  carrying its answer, against its own library's stated contract that *"stdout is the ANSWER"*
...
| `--base` | string | no | the repository's default branch | passed through to the live re-derivation |
| `--repo` | string | no | the `origin` remote's `owner/name` | the repository |

**Output** — machine. One JSON object:

```json
{"issue":5021,"pack":"sealed","comment":9234567891,"nonce":"7f3a9c21","sealedAt":"2026-08-09T18:36:48Z","author":"usirin","asserted":{"intent":"Widen the fanout guard.","established":"The guard reads one file only; a failing case is committed.","nextAct":"Follow one level of local helper call, then re-run the case.","unsure":"Whether one level is enough."},"ground":{"packed":"a1b2c3d4e5f6","live":"b2c3d4e5f6a1"},"drift":{"state":"moved","fields":[{"field":"git.head","packed":"4f1c8a2b9d3e5607182934abcdef5566778899aa","live":"7ab3419e0c25d8f6041a2b3c4d5e6f7089abcdef","state":"moved"},{"field":"board.pull.checks","packed":"failing","live":"passing","state":"moved"}]},"claim":null,"disregarded":[],"scanned":{"comments":14}}
```
...
| `sealed` | the latest pack parsed and carries no claim |
| `claimed` | the latest pack parsed and a claim marker holds it; `claim` carries the holder's nonce and timestamp |

When `pack` is `none`, `comment`, `nonce`, `sealedAt`, `author`, `asserted`, `ground`, `drift` and
`claim` are all `null`, and `disregarded` says whether anything was *nearly* a pack.

**`drift.state` is a closed set of three**, and is the **worst** of the per-field states: `none`
when every field compared equal, `moved` when at least one differs and none is unknown, `unknown`
when any field's live value could not be derived. `fields` lists **only** the fields whose state is
not `same`, each with `packed`, `live` and its own `state`. The comparison is field-by-field over
the ground-state table, so the derivation is fully specified there and not restated here.
...
`unknown` is a third answer for the same reason `no-evidence` is: a comparison that could not be
made is not a negative result.

<!-- anchor: THE-AUTHOR-IS-RESOLVED-NOT-TRUSTED --> **A pack's author is resolved against
repository permissions before the pack is honoured.** A comment carrying a well-formed marker is
still a comment anyone with a GitHub account can post, and its `## Next act` is a sentence a
successor reads and then acts on — the highest-leverage read-to-write path in the quintet. An
author who does not resolve to `write` or above lands in `disregarded` with reason `unauthorized`
and the verb keeps looking at older packs; a permission read that **fails** is `11` for the whole
call, never a grant (ADR 0055).

**`disregarded`** is an array, empty when nothing was disregarded, of every comment this verb did
not honour as the pack: `comment` (its id), `reason` (a closed set — `unauthorized`, `superseded`),
and `detail`, a human-readable string. A pack that *looks* present to a human is therefore visible
to the reader rather than silently absent.

<!-- anchor: A-MALFORMED-LATEST-PACK-REFUSES --> **A malformed pack is `14` and never `none`.** The
`WireRead` three-valued read is what makes the distinction available, and honouring it matters more
here than anywhere else in the group: reporting `none` over a pack that exists tells a successor
nobody handed off, and it starts the work over. The scope is the **latest** pack specifically —
falling back to an older one would hand a successor a stale pack believing it current, which is the
same #3330 failure wearing a recovery's clothes. So the older pack is not read, and the refusal
names the comment a human must look at.
...
number disregarded. **Zero comments is a fact** (`pack: "none"`); a page that could not be fetched
is `11`, never an empty comment list — an unpaginated read that missed the newest pack would report
...
- The same tool disables its own dangling-reference check whenever the sub-issue read returns empty
  (`validate.ts:130`, `if (subIssues.length > 0)`), so a rate-limited call silently removes the
  check rather than refusing. Here an empty read that cannot be proven empty is `11`.
- `intake-dedup` exits `0` whether or not it found anything and writes its no-usable-keywords case
  to stderr before returning clean, so a caller reading only the status cannot tell "no duplicates"
  from "the check never ran". That is the shape this verb's `pack` token exists to avoid: the
  answer is a word on stdout, not an inference from an exit code.
...
| `--nonce` | string | yes | — | this run's nonce, matching `^[0-9a-f]{8}$` — the claim key. A value outside that grammar is a usage error (`1`), so a human-readable label like `run-1`, which two runs would collide on, is refused rather than quietly shared |
| `--repo` | string | no | the `origin` remote's `owner/name` | the repository |

**Output** — machine. One JSON object:

```json
{"issue":5021,"pack":9234567891,"nonce":"4b8e2f01","claim":"held","claimedAt":"2026-08-09T19:02:11Z","comment":9234599999}
```
...
is generated once per run by the caller and passed explicitly, which is also what keeps it out of
...
read that fails is `11` for the whole run**, never a demotion that would silently free someone
else's claim.
...
```
$ fabrika handoff claim --issue 5021 --nonce 4b8e2f01
handoff claim: pack #9234567891 is held by 7f3a9c21 since 2026-08-09T19:02:11Z — refusing to open a second claim on one pack.
$ echo $?
15
```
...
- #5283 — a retiring crew seat wrote a successor checkpoint with no findable home; a booting seat
  spent ten minutes on transcript archaeology and two recovered findings fired within the hour.
  The motivating incident: the claim is what tells a third seat that a second is already on it.
...
table, and **front-door's detection parses that one** ([#4952](https://github.com/kamp-us/phoenix/issues/4952)).
This table is the same shape and the same four rows, scoped to the reads **these verbs** make; it
adds no row the skill's table does not already carry. When-missing vocabulary is the closed set
...
**First-run behaviour in a fresh repository.** Every refusal above names a way forward, and none of
them is a first-run dead end: an issue with no pack is `read`'s ordinary `none` at exit `0` rather
than a refusal, there is no label to bootstrap, and the first `handoff take` on a repository that
has never seen one needs nothing that does not already exist. The only first-run friction is a
repository with no `origin`, which degrades rather than blocks.

## Self-test against the completeness test

1. **Every flag has a type and, if optional, a default** — the four Inputs tables.
2. **Every stdout shape is shown by an example** — four Examples blocks, plus the pack document in
   *The pack document this group WRITES*.
3. **Every non-zero exit code is enumerated with the condition that produces it** — the shared
   matrix owns `code → meaning`; each verb's Exit status owns its own triggers.
4. **Every error names its message, its stream and its code** — four Errors tables, all stderr.
5. **Every judging verb states its scope and its zero-scope behaviour** — `take`, `read` and
   `claim` each carry a Scope block naming what a failed read costs; `capture` states that a failed
   git read is `11` rather than a clean tree.
6. **No clause defers to a v1 script, another skill's prose, or the authoring session.** Every v1
   ...
   group borrows no verb from `map`, `grill` or `spike`, which is what keeps the four
   independently implementable while `graduate` is still unmerged.
7. **Every value an example prints is derivable from the spec** — the ground state's fifteen fields
   each name their derivation, `groundDigest` names its exact input and truncation, `drift` is a
   field-by-field comparison of two records specified above, and the digests and nonces printed in
   examples are declared as grammar-conforming placeholders in the header rather than as computed
   values.

**Every value a later verb needs arrives as an argument.** `--nonce` is passed to `take` and
`claim`, never inferred; `--issue` addresses every verb; and `read` re-derives the live ground
itself rather than being handed one. No verb depends on state that exists only in the calling
session's memory — which is the one property this group could least afford to get wrong, given what
it is for.
