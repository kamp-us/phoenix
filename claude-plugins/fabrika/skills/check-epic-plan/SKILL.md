...
that terminal: this verdict first, then `fabrika build note $epic_number --token $claim_token` with the un-flipped refs.** The note's
...
local, or remove.** Release the claim with `fabrika build release $epic_number --token $claim_token` on every terminal reached
...
- `FLIP-PARTIAL` — `22`: the floor was clean and something did not move — some children, or the
  ...
  named with `fabrika build note $epic_number --token $claim_token` — refs only, no claim about what any issue carries now (step 3);
...
- `STOPPED` — everything else that leaves the run UNKNOWN with nothing written: `4`, `11`, `23`, a
  ...
  state for a successor with `fabrika build note $epic_number --token $claim_token` **when you hold the claim**; when the claim itself
