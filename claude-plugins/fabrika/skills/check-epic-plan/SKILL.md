...
would be admitting itself. Triage never writes it either — it once did, which made the label say
"triaged" rather than "gated" and let an ungated epic read pickable, so
`triage apply --type epic --ready-for agent` now stamps no audience label at all. Only this gate has
already proven the floor clean, so only this gate may write it, and the verb writes it **last**, after every child's re-read proves it moved: an epic that
