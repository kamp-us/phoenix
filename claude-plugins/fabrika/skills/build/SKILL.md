...
The pool is `status:triaged` + `ready-for:agent` + unassigned, p0 first. An assigned issue is not
yours whatever its labels — assignment is how humans keep documents out of this pool (#4764).
...
`won` prints your token; `lost` names the winner — that lane is theirs, back off. Before **every**
