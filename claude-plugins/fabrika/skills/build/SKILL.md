...
| The CI gate workflow — `.github/workflows/<ci.gateWorkflow>`, defaulting to `ci.yml` | It is the superseding authority over `build check`'s in-tree prediction (`fabrika wire doc-section --heading "build check" < <skill-base>/contract.md`) | **degrade** — with no CI gate to supersede it, `build check`'s green is the only evidence the PR carries, and the PR says so in its `## Deviations`. A repo whose gate has another filename declares it under `ci.gateWorkflow`; zero workflows at all is `ci.noProducer`'s question, and its shipped `refuse` is what `review ci` and `ship checks` answer with. |
