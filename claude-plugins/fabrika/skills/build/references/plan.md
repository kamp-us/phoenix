...
- **The `## Dependencies` topology is explicit and complete** — phases and `requires:` edges
  ...
  ledger's shape off; the gate itself reads the tracker's own native blocked-by edges, so an edge
  that exists only in this prose is an edge no verb enforces — a plan whose ordering lives in
  paragraphs lets a child be picked before the work it needs has landed.
