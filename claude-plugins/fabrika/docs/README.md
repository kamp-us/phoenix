...
| [hook surface](hook-surface.md) | where a fabrika hook is declared ([`../hooks.json`](../hooks.json)) and how it invokes a verb, the harness exit-code contract, the ruled dispatch-failure policy point, and the record format every grading child writes into |
| [hook records](hook-records.md) | the graded half of the hook layer — the proven/unproven account of the ruled delivery channel, and one recorded verdict per graded piece in the format [`hook-surface.md`](hook-surface.md) specifies |
