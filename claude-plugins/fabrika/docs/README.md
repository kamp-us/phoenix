...
| [§CP classification](control-plane-classification.md) | the ruled control-plane model every §CP-computing verb implements and every §CP-mentioning skill is held to — CODEOWNERS as single source, teams and individual `@login`s alike, three-valued, an absent boundary is no control plane and an unreadable one follows the repo's own `unreadableCodeowners` declaration, no semantic detection | #4932 |
