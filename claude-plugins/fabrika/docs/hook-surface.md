...
**Owner:** `fabrika hook spawn`, declared in [`../hooks.json`](../hooks.json). The dispatch-failure residue is [the ruled policy point](#the-dispatch-failure-policy-point) plus the `127` state the notice cannot cover, which installing the now-published package removes per machine but no ticket discharges; the ungraded `statusline` mode has no owner yet.
