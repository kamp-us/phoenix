...
1. **Isolate.** Work in a per-run git worktree so nothing touches the user's tree *and* two concurrent decomment passes never collide on the branch or the dir — uniquify both with a per-invocation suffix (`<prefix>` is your personal branch prefix, e.g. `umut/`): `SUF="$(date +%s)-$$"; git worktree add -b "<prefix>/decomment-$SUF" "../phoenix-decomment-$SUF" main`. (A hardcoded `<prefix>/decomment` / `../phoenix-decomment` makes the second run fail with `'<prefix>/decomment' is already checked out` or clobber the shared dir.) Everything — this cleanup, plus any new ADR/pattern docs — lands on that one branch as one reviewable PR.
