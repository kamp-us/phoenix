...
- **Node over Python for scripts/hooks.** Mechanical tooling lives as an Effect CLI package under `packages/` (the `epic-ledger` / `crabbox-manifest` / `leak-guard` idiom — `effect/unstable/cli`, run with `node src/bin.ts`), not an ad-hoc script or a Python hook. A pure, unit-tested core + a thin Effect bin; never a one-off `.py`. The pipeline tools fold into one router, `pipeline-cli <tool> …` ([`packages/pipeline-cli/`](packages/pipeline-cli/)); before hand-rolling `gh`/`jq`/`git` glue, reach for an existing tool — for the full one-line-per-tool `name · purpose` map **on demand**, run `pipeline-cli commands compact` (generated from the registry, never auto-injected; mirrors the ADR-discovery `decisions-index compact` contract below). Sanctioned exception (founder ruling, 2026-07-29): a sourced orchestration script under `claude-plugins/kampus-pipeline/skills/**` that calls existing `pipeline-cli` verbs — bounded by ADR [0228](.decisions/0228-scripts-relay-never-derive.md), a script may relay a verb's answer, never derive the decision itself. Such a script is written in the bash-3.2 shape recorded in [`.patterns/skill-script-shell-shape.md`](.patterns/skill-script-shell-shape.md) — `set -uo pipefail` **never** `-e` (errexit plus a cleanup `EXIT` trap launders a `set -u` abort into exit 0, so the guard passes on the path it aborted in), defaulted array expansions, and an emptiness test before expanding an array into an argument list (#4479). No CI job enforces that shape any more — `trap-status-guard.yml` retired in #6098 along with the rest of `pipeline-cli`'s self-policing guards, because `claude-plugins/fabrika/**` carries no `.sh` file and no extracted-script idiom for it to police.
