 ...
 * is a list *of* is proven without a DOM in `@kampus/tuval-ui`'s `subagents.unit.test.ts`. It lives in
 * the app because it drives the shell's own core machine (`../shell/core/machine.ts`).
 *
 ...
 * (`../shell/ui/press.ts`'s reasoning, and `@kampus/tuval-ui`'s `subagent-navigator.unit.test.tsx`).
 */
...
import {ForwardedKeyProvider} from "@kampus/tuval-ui/forwarded-key";
import {defaultPrefixTable, FOCUS_LIST_KEY} from "@kampus/tuval-ui/keys";
import {call, userItem, withTranscript} from "@kampus/tuval-ui/testing/chat";
import {installDomShims} from "@kampus/tuval-ui/testing/dom";
import {act, fireEvent, render, screen} from "@testing-library/react";
import {Effect} from "effect";
import type {ReactElement} from "react";
import {describe, expect, it} from "vitest";
import {applyMsg, initialState, openProcessMsg} from "../shell/core/machine.ts";
import {activeWorkspace, type ShellState, windowIds} from "../shell/core/state.ts";
