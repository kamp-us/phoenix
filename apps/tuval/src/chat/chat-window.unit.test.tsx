 ...
 *
 * The chat window, rendered against the window contract's own test double
 * (`@kampus/tuval-sdk/kernel/shell/window/fixtures`) — the same `WindowHost` the WebSocket transport
 * implements, which is the whole point of the seam being transport-blind. No kernel, no socket, no
 * agent layer appears here. The window is `@kampus/tuval-ui`'s; this suite lives in the app because
 * both channels round-trip through the desk's own wire codec (`../shell/transport/wire.ts`).
 *
...
import {
	...
	userItem,
	withTranscript,
} from "@kampus/tuval-ui/testing/chat";
import {growObservedElement, installDomShims, TEST_VIEWPORT} from "@kampus/tuval-ui/testing/dom";
...
describe("keys typed on the transcript", () => {
	 ...
	 * (`../shell/ui/Desk.tsx`) — the window is mounted under `body` here, and there is no desk above it.
	 */
	const armTheDesk = (): void =>
		globalThis.document.body.setAttribute(PREFIX_ARMED_ATTRIBUTE, "true");

	/** Every keydown the desk's one document listener would have seen (`../shell/ui/Desk.tsx`). */
