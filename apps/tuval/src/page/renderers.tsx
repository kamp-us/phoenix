 ...
 *
 * The Pi entry is `PiChatWindow` (#7611), the Claude entry is `ClaudeChatWindow` (#7624) and the agy
 * entry is `AgyChatWindow` (#8180), and they are why this module is out of the kernel's strict lens
 * and inside `tsconfig.design.json`'s: the chat window is built on `@kampus/design`, which is
 * source-consumed and authored with `exactOptionalPropertyTypes: false`. Each key is the reference
 * that program's own row declares, imported rather than retyped, so a row and this table cannot name
...
export const pageRenderers: Readonly<Record<string, AnyWindowRenderer>> = {
	...
	[AGY_CHAT_WINDOW_REF.ref]: AgyChatWindow,
};
