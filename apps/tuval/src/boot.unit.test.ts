	...
	it(
		...
		async () => {
			...
			expect(first.stdout).toContain(
				`tuval: booted — 9 program(s), ${BOX_SPELLS} spell(s) registered from ${boxConfig}; 3 process(es) live, 0 restored from ${projectDir(project)}\n`,
			);
			expect(first.stdout).toContain(
			...
			expect(second.stdout).toContain(
				`tuval: booted — 9 program(s), ${BOX_SPELLS} spell(s) registered from ${boxConfig}; 3 process(es) live, 3 restored from ${projectDir(project)}\n`,
			);
