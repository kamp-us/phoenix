...
const kernel = (rows: ReadonlyArray<AnyProgram>, stores: CheckpointStores) =>
	Processes.layer.pipe(
		...
		// Merged, not provided: `restore.ts` reads the row back to ask it what a restored process
		// should be sent, so the Registry has to still be in the context it runs under.
