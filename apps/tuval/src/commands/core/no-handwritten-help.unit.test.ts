...
// `fixtures.ts` omits `processSpells`) would let that group's sentences be copied in unnoticed.
const sentences = coreSpells.map((spell) => spell.describe);

describe("the core spell sources", () => {
	it("covers every spell boot registers as core", () => {
		expect(sentences.length).toBeGreaterThan(0);
		expect(new Set(sentences).size).toBe(sentences.length);
	});

	it("greps clean: no sentence is written in a file that does not define the spell", () => {
