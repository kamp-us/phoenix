 ...
 *
 * `packages/tuval-claude/src/tools/server.ts` turns a `KernelBridge` into the MCP server the SDK mounts, and
...
import {
	...
	tuvalToolServer,
} from "@kampus/tuval-claude/tools";
...
export const claudeTools = Effect.fn("Tuval.ClaudeVerticalProof.claudeTools")(function* (
	...
	// `ClaudeAiAgent`'s own shape (`packages/tuval-claude/src/agent/ClaudeAiAgent.ts`): the SDK gives no hook for handing it
