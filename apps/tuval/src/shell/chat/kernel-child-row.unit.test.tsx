...
const flagged = (opened: Array<string>): ChatWindowOptions => ({
	subagentList: true,
	kernelChildren: true,
	openProcess: (processId) => opened.push(processId),
});

describe("a kernel child in the sub-agent list", () => {
	it("is a row of the list beside the backend's own worker", async () => {
		const {rendered} = await openWindow(both(), flagged([]));
	...
	it("opens as its own window when activated, where a worker's row swaps the view", async () => {
		...
		const {rendered} = await openWindow(both(), flagged(opened));

		await click(rows()[1] as HTMLElement);
		expect(opened).toEqual(["p-9"]);
		// The window stayed on the agent's own transcript: no way-back row was added.
	...
	it("reaches both by keyboard, through the handlers the mouse calls", async () => {
		const opened: Array<string> = [];
		const {rendered, chord} = await openWindow(both(), flagged(opened));
