/**
 * Where `@kampus/tuval-sdk`'s source sits on disk, for the tests here that read a kernel module's text
...
const manifestPath = require.resolve("@kampus/tuval-sdk/package.json");
...
/**
 * The file a `@kampus/tuval-sdk/…` specifier loads, read off the exports map the way a bundler reads it
...
export const resolveSdkSpecifier = (specifier: string): string | undefined => {
	if (!specifier.startsWith("@kampus/tuval-sdk/")) return undefined;
	const subpath = `.${specifier.slice("@kampus/tuval-sdk".length)}`;
