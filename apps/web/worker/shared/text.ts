...
/**
 * Lowercase, hyphenated, ASCII-safe slug with Turkish characters folded to
 * their closest Latin counterparts (`ı→i`, `ş→s`, …). Mirrors the
 * `slugifyTurkishTitle` helper that `monorepo/packages/odin-content/utils`
 * settled on — the seed importer and any future user-driven term/post creation
 * land here.
 *
 * Idempotent: `slugify(slugify(x)) === slugify(x)` for any input.
 *
 * @example
 *   slugify("Ağaç Yapısı")     // "agac-yapisi"
...
export function slugify(input: string): string {
	return input
		.replace(/[İıĞğÜüŞşÖöÇç]/g, (m) => TURKISH_FOLD[m] ?? m)
		.toLowerCase()
		.replace(/[^a-z0-9\s-]/g, "")
		.trim()
		.replace(/[\s-]+/g, "-");
