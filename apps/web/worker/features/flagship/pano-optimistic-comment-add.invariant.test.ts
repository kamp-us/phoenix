/**
 * The dark-ship default-=-safe-state invariant for the optimistic `comment.add`
 * containment flag (#1678, epic #1637). Inspected off the exported
 * `PANO_OPTIMISTIC_COMMENT_ADD_FLAG` record (the same object the factory spreads
 * into `FlagshipFlag`), so no alchemy resource is constructed — mirrors
 * `pano-optimistic-submit.invariant.test.ts`.
 */
