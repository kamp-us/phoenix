 ...
 * includes the returned statement inside its `Drizzle.batch(...)` so the
...
import type {DrizzleDb} from "../../services/Drizzle";

/**
 * Build the unexecuted `UPDATE user_profile SET total_karma = total_karma + ?`
 * statement. `delta` may be negative (vote retraction).
 *
 * The Vote service uses this inside `Drizzle.batch(...)` — never call `.run()`
 * on the result. If you need a one-off karma bump outside a batch, wrap the
 * call in `Drizzle.run((db) => karmaBumpStatement(db, ...))`.
 */
