...
import {decideConfirmGone, decideReadback, type ReadbackState} from "./readback";
...
describe("decideConfirmGone", () => {
	...
	it("settles on an empty connection (the deleted node was the last one)", () => {
		expect(decideConfirmGone(new Set<string>(), state("c2", 3))).toEqual({action: "settled"});
	});

	it("waits while probes remain and the node is still present", () => {
		expect(decideConfirmGone(new Set(["c1", "c2"]), state("c2", 3))).toEqual({
			action: "wait",
			next: {expectedId: "c2", probesRemaining: 2},
		});
	});

	it("counts probes down to zero across successive still-present ticks", () => {
		let s = state("c2", 2);
		const present = new Set(["c2"]);
		const first = decideConfirmGone(present, s);
		expect(first).toEqual({action: "wait", next: {expectedId: "c2", probesRemaining: 1}});
		s = (first as {next: ReadbackState}).next;
		const second = decideConfirmGone(present, s);
		expect(second).toEqual({action: "wait", next: {expectedId: "c2", probesRemaining: 0}});
	});

	it("refetches deterministically once the budget is spent and the node lingers", () => {
		expect(decideConfirmGone(new Set(["c2"]), state("c2", 0))).toEqual({action: "refetch"});
	});

	it("settles even on the final probe if the push lands at the last moment", () => {
		// Absence is checked before the budget — a late deleteEdge still wins over a refetch.
		expect(decideConfirmGone(new Set(["c1"]), state("c2", 0))).toEqual({action: "settled"});
	});

	it("treats a negative budget as spent (refetch, not an infinite wait)", () => {
		expect(decideConfirmGone(new Set(["c2"]), state("c2", -1))).toEqual({action: "refetch"});
	});
});
