...
export type CommentData = {
	...
	/**
	 * `true` when the comment row is the soft-deleted-with-replies
	 * `[silindi]` placeholder (task_12). The card stays in the tree to preserve
	 * thread structure, but the vote button + reply / edit / delete affordances
	 * are hidden — there's no live author to act against.
	 */
	isDeleted?: boolean;
	...
	/**
	 * Optional inline edit composer rendered IN PLACE OF the comment body
	 * (task_12). Owned by the page so it can wire to the `editComment`
	 * mutation + refetch. When present, the static body is hidden; reply and
	 * delete affordances stay reachable so the user can cancel out via the
	 * composer's own cancel button.
	 */
...
export function PanoComment({
	...
	const editing = !!comment.editComposer;
	...
	return (
		<article className={cls} id={comment.hash}>
			...
			{open ? (
				<>
					{editing ? (
						<div className="kp-comment__edit" data-testid={`pano-comment-edit-${comment.id}`}>
							{comment.editComposer}
						</div>
					) : (
							...
							{comment.isOwner ? (
								<Menu.Root>
									<Menu.Trigger
										...
										data-testid={`pano-comment-menu-${comment.id}`}
									>
										⋯
									</Menu.Trigger>
									<Menu.Popup align="start">
										<Menu.Item
											...
											data-testid={`pano-comment-edit-trigger-${comment.id}`}
										>
											düzenle
										</Menu.Item>
										...
										<Menu.Item
											...
											data-testid={`pano-comment-delete-trigger-${comment.id}`}
										>
											sil
										</Menu.Item>
									</Menu.Popup>
