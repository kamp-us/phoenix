...
A domain error becomes a wire error by carrying a `WireCode` annotation. That's the whole
...
| Module | What it is |
| --- | --- |
| `WireError.ts` | `WireCode` annotation + `encodeWireError` (the one error codec) |
...
| `Fate.ts` | the `Fate` authoring namespace (the constructors + `Entity` + `WireCode`; every member is also flat-exported) |
