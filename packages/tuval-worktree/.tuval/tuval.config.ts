 ...
 * here — `worktree.unit.test.ts` imports it and reads the row back — so the usage in the README is
 ...
 * `@kampus/tuval-sdk/kernel/config` publishes. A config written outside the repo is therefore checked
...
export const desk = worktree({
	repo: "/tmp/tuval-worktree-fixture",
	base: "origin/main",
	...
	teardown: [],
	runner: inert,
	job: claudeSession({cwd: "/tmp/tuval-worktree-fixture", scope}),
});
 ...
 * half of the surface: a worktree program that only provisions.
 */
export const reviews = worktree({
	id: "reviews",
	repo: "/tmp/tuval-worktree-fixture",
	root: "/tmp/tuval-worktree-fixture/.reviews",
	branchPrefix: "review/",
	env: false,
	runner: inert,
});
...
export default {
	...
	graph: {
		nodes: [
			{id: "worktree", program: desk.id, on: []},
