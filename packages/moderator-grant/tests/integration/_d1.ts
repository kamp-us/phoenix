/**
 * Per-file real-D1 substrate for the grant integration tier — the alchemy `Test.make`
 * idiom of ADR 0082 / `.patterns/alchemy-test-harness.md`, scoped to a **D1-only
 * stack** (this package has no worker; the grant CLI talks to D1 directly). The twin
 * of `@kampus/preview-seed`'s `tests/integration/_d1.ts`, returning a `grantDb()`
 * accessor instead of `seedDb()`.
 *
 * Each integration test file calls `grantD1(import.meta.url)` once at module top
 ...
 * `0007_moderation_role_resolution.sql` that adds `user.role`) — and returns a
 * `grantDb()` accessor that builds a `GrantDb` over the production REST transport
 * (`makeD1Rest` + `makeGrantDb`) pointed at that stage's real D1. The grant then runs
 * the SAME `setRole`/`listModerators` path the bin ships; assertions read back over
 * the same REST seam.
 *
 ...
 * apps/web / preview-seed use (`_stage-name`'s run-unique `it-<readable>-<disc>`).
...
// `packages/moderator-grant/tests/integration/` → repo root is four levels up.
 ...
 */
export function grantD1(metaUrl: string): GrantD1 {
	...
	const target = (): {accountId: string; databaseId: string} => {
		if (!resolved) {
			throw new Error(
				"D1 not deployed — beforeAll(deploy) has not resolved. Build the harness via grantD1().",
			);
		}
