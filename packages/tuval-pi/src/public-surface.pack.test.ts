/**
 * The published surface of `@kampus/tuval-pi`, read off a real `pnpm pack` rather than off the
...
describe("the packed @kampus/tuval-pi", {timeout: SUBPROCESS_TEST_TIMEOUT_MS}, () => {
	beforeAll(() => {
		const tmp = mkdtempSync(join(tmpdir(), "tuval-pi-pack-"));
	...
	it("is public under its published name", () => {
		expect(manifest.name).toBe("@kampus/tuval-pi");
