...
/**
 * Pi's own reply builders, re-exported so a desk proof scripts its replies through this door and
 * lists no Pi package of its own: the Pi dependencies are this package's.
 */
export {fauxAssistantMessage, fauxText, fauxToolCall} from "@earendil-works/pi-ai";
			...
			Layer.unwrap(
				Effect.tryPromise({
					try: async () => {
						...
						// The same flag the shipped row reads (`../ai-agent/PiAiAgent.ts`), so flipping it and
						// running the desk app's `proof:pi-vertical` is the hand check for the extension
						// (#8555).
