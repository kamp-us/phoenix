 ...
 * and left out of both picker lists (`apps/tuval/src/shell/picker/entries.ts`). The row declares
 ...
 *
 * `boot` reads a project's config at `<root>/.tuval/tuval.config.ts` (`apps/tuval/src/boot.ts`), so the root
