/**
 * The published surface of `@kampus/tuval-claude`, read off a real `pnpm pack` rather than off the
...
const DOORS = [
	...
	"./window",
	"./testing/window",
	"./testing/scripted-query",
	"./testing/history-fixtures",
	"./testing/initial-effort",
	"./testing/paging-replay",
	"./package.json",
];
...
describe("the packed @kampus/tuval-claude", {timeout: SUBPROCESS_TEST_TIMEOUT_MS}, () => {
	beforeAll(() => {
		const tmp = mkdtempSync(join(tmpdir(), "tuval-claude-pack-"));
	...
	it("is public under its published name", () => {
		expect(manifest.name).toBe("@kampus/tuval-claude");
