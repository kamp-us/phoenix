 ...
 * and `<host>` is a fingerprint of the machine it runs on. That stamp is what turns "is this claimant still
 ...
 * stamp (a legacy marker), a stamp from another host, an unresolvable local machine identity, or
 * an unprobeable pid all resolve `unknown`, which counts as a live claim — so an indeterminate
 ...
 * of a Linux `/etc/machine-id`, which systemd's own man page declares confidential and directs
