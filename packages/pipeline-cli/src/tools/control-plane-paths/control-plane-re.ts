 ...
 * single always-on required status context on `main` — moved out of the pipeline-cli `ci-required`
 * tool and into `packages/fabrika-cli/src/ci/` (#6099), while the pipeline-cli copy stayed as
 * the frozen v1 baseline nothing runs (ADR 0238). So the covered path went dead and the live one was
 * uncovered: a PR touching only the new dir matched no CODEOWNERS row, and with no `*` catch-all and
 * the ruleset at `required_approving_review_count: 0` + `require_code_owner_review: true`, that merges
 * at ZERO approvals. Under ADR 0187's enforcement test the core stays §CP wherever it runs. The rest
 * of `packages/fabrika-cli/` is ordinary by founder ruling on #6164 — verbatim: "i'm ok with it as
 * long as it's scoped to the ci checks, if not i trust fabrika enough to self drive itself". Recorded
 * in ADR 0299, extending ADR 0218's enforcement-core shape to fabrika-cli.
 *
