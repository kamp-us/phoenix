...
const rollup = leafCommand(
	...
	Command.withDescription(
		"What fabrika's runs cost, summed out of the durable spend ledger, spawning nothing and re-parsing no transcript. stdout is one record per line, first field naming the kind: billed, exCacheRead, assistantTurns, runs, measuredRuns, then skipped/skippedMalformed/skippedNewerVersion/undatedRows. These scalar lines are `<kind>\\t<count>`. Each breakdown row starts with `day\\t<day>`, `skill\\t<skill>` or `stage-arm\\t<stage>\\t<arm>`, followed by five tab-separated totals in order: billed, exCacheRead, assistantTurns, runs, measuredRuns. At most ten rows appear per breakdown, its biggest-billing rows first, followed by a `dayMore`/`skillMore`/`stageArmMore` count of the rows the cap dropped, printed even at `0`. skippedMalformed counts rows lost to data damage; skippedNewerVersion counts rows this CLI is too old to read. --since/--until bound the window inclusively; a bare YYYY-MM-DD widens to the whole UTC day. --json emits {window, totals, skipped, undatedRows, byDay, bySkill, byStageArm}. window has since/until strings or null for unbounded edges; totals has the five named totals; skipped has total/malformed/newerVersion counts. Each of byDay/bySkill/byStageArm is {rows, more}, where rows contains objects with the five named totals plus day, skill, or stage and arm respectively, and more counts omitted rows under the same cap and ordering. Exits 7 (no ledger there — nothing recorded yet), 11 (the ledger could not be read — the spend is UNKNOWN, never zero), 12 (read in full, no rows at all), 13 (rows exist but this window selects none). No threshold and no budget flag: measuring what the runs cost must never become a way to fail one. The breakdowns are capped so an ever-growing ledger cannot print an ever-growing answer. Example: fabrika spend rollup --since 2026-08-01",
	),
);
