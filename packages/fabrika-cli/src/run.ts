/**
 * The fabrika-cli router wiring — the real bin body, behind `bin.ts`'s bootstrap.
 *
...
const cli = Command.make("fabrika-cli").pipe(
	Command.withSubcommands(registeredGroups),
	Command.withDescription(
		"fabrika's deterministic verb package — `fabrika-cli <group> <verb> …`. Stdout is the answer; scope lines and refusals go to stderr; a non-zero exit is UNKNOWN, never a partial answer.",
	),
);
