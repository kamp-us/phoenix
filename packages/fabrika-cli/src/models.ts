 ...
 * imports it and nothing declares a second copy — a `--model` flag normalizes its alias against this
 * same table rather than carrying one of its own. Reimplemented from v1's `spawn-guard`, never
 * imported from it: v1 is frozen and may not be called, so its scars are carried across as behaviour
 * and as the tests below, not as a dependency.
 */
 ...
 * exists to permit, which is the defect v1 shipped. Only the Opus-4.8 family has an entry —
 * `sonnet` and `haiku` are deliberately absent, so they canonicalize to themselves, miss the
 * allowlist and stay denied.
 */
...
/**
 * The committed pin an absent `WORKFLOW_MODEL` falls back to.
 *
