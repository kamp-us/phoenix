...
const evidence = leafCommand(
	...
	Command.withDescription(
		"Read the SHA-bound run-evidence bundle as four states. First stdout line is `evidence\\t<present|pending|absent|unknown>\\t<sha>`, then `lookup\\trun:<id|->\\tartifact:<id|->\\tstatus:<status|->` — the evidence that makes the claim falsifiable — then one `check\\t<name>\\t<status>` line per manifest entry. Pending is not absent (#3913), a failed read is not absent (#3716), and the fetched artifact's zip magic number is checked before anything parses it. Exits 7 (PR or --sha proven absent), 11 (the run list, artifact list or artifact content could not be read after retries), 13 (a run or artifact enumeration is provably short). Example: fabrika ship evidence 4321 --sha 03135b91",
	),
);
...
const enqueue = leafCommand(
	...
	Command.withDescription(
		"Arm the merge queue's auto-merge at a pinned head and prove the arm landed. Prints `enqueued\\t<sha>\\t<queued|settling>` — `settling` is the normal race, not a failure. There is NO merge-method flag by construction: the queue owns the method, and a method flag alongside the arm silently no-ops the enqueue at exit 0. `auto_merge: null` post-enqueue is expected and is never read as a jam. Exits 7 (PR proven absent, closed or already merged), 8 (the arm or its confirming read-back failed — whether an intent is parked is UNKNOWN, so run `fabrika ship disarm --site refuse` before stopping), 11 (the live head could not be read — nothing was armed), 12 (the live head moved past --sha — refusing to arm a tree nobody verified). Example: fabrika ship enqueue 4321 --sha 03135b91",
	),
);
...
const reconcile = leafCommand(
	...
	Command.withDescription(
		"Watch a queued PR to a terminal classification. Prints `reconcile\\t<landed|ejected|unresolved|parked>\\t<polls-used>\\t<horizon-seconds>` — all four are proven answers at exit 0, so no loop shape around this verb can launder an ejection into a success (#4557). `landed` on merged:true or a base-branch squash whose subject ENDS with `(#<pr>)`; `ejected` only on a removal NOT paired with a merge (#4155); `unresolved` means still-queued at the horizon, the expected outcome of a healthy long dwell; `parked` means the arm never entered a queue on a queue-governed base. Exits 7 (PR proven absent), 11 (every poll failed to read — UNKNOWN, distinct from `unresolved`), 13 (a timeline enumeration is provably short). Example: fabrika ship reconcile 4321",
	),
);
