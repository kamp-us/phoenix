...
const cpApproval = leafCommand(
	...
	Command.withDescription(
		"Discharge the §CP approval cardinality table at one head: `cp-approval\\t<discharge|stop|n/a>\\t<mechanism>`. The roster is the control-plane team parsed off .github/CODEOWNERS, read fresh; every discharge signal is head-bound, so a stale approval on a superseded head never counts. A failed read is UNRESOLVED, never `stop` and never `awaiting-approval`. A stderr notice fires when the banked head is behind the base. Exits 7 (PR proven absent or closed), 11 (the boundary, roster, reviews, markers or live head could not be read), 13 (the comment enumeration is provably short of the declared count, or the review read — which the platform gives no count for — never reached a terminal page with no `next` link). Example: fabrika ship cp-approval 4321 --sha 03135b91",
	),
);
...
const enqueue = leafCommand(
	...
	Command.withDescription(
		"Arm the merge queue's auto-merge at a pinned head and prove the arm landed. Prints `enqueued\\t<sha>\\t<queued|settling>` — `settling` is the normal race, not a failure. There is NO merge-method flag: the queue owns the method. `auto_merge: null` post-enqueue is expected and is never read as a jam. Before the arm, a DEFINITE and TRUE `mergeable` is asserted: `mergeable` is computed lazily so an indefinite value is polled, and if it is still indefinite it is UNKNOWN and refuses on 11. A definite not-mergeable read splits by cause. A `mergeable_state: dirty` is a fact about the BASE, so this verb rebases the PR's head branch onto its base in a scratch worktree, publishes it under a lease, re-reads mergeability and arms the replayed head — the printed `<sha>` is then that head, `--json` says `\"rebased\":true`, and no repair round is reported. No re-review is owed: a verdict binds to content, not to a head SHA (src/review/head-content.ts). A replay that cannot apply changes content and refuses on 16, as does every other definite not-mergeable state and any dirty PR no replay was attempted on (fork head, no remote for the repository, an intent already parked). Exits 7 (PR proven absent, closed or already merged), 8 (the arm or its confirming read-back failed — whether an intent is parked is UNKNOWN, so run `fabrika ship disarm --site refuse` before stopping), 11 (the live head or the mergeability could not be read, mergeability stayed indefinite, or a published rebase could not be read back — nothing was armed), 12 (the live head moved past --sha — refusing to arm a tree nobody verified), 16 (the PR is provably not mergeable and no rebase resolves it — nothing was armed). Example: fabrika ship enqueue 4321 --sha 03135b91",
	),
);
...
const release = leafCommand(
	...
	Command.withDescription(
		"Detect a dark ship over the PR itself and queue its linked issue for release. Prints `release\\t<queued|n/a|no-issue>\\t<flag-key|->`. Three ground-truth signals, any one sufficient: the diff adds a flag declaration, the body carries a `Flag:` line, or the body names a key declared in the registry at the base branch inside a gating-context line. The linked issue's inherited `Containment:` stamp is NEVER read — it describes the epic, not this PR. `no-issue` is its own answer, never folded into n/a. Agents deploy, humans release — this ends at the label. Exits 7 (PR proven absent), 8 (the label write or its re-read failed — escalate), 9 (it landed and the read-back does not show it), 11 (the diff, body, registry or linked issue could not be read — dark-ship-ness is UNKNOWN, never n/a), 13 (the changed-file enumeration is provably short). Example: fabrika ship release 4321",
	),
);
