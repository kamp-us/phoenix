...
const ciProduce = leafCommand(
	...
	Command.withDescription(
		"Internal trusted-workflow leg: the image build performs only fixed-tool setup and a scriptless dependency fetch, then offline PR lifecycle install plus test run under read-only-root/capability-drop/no-network isolation and the server under the same root/capability/no-network restrictions with its installed workspace read-only and no published port. A base-owned capture sidecar shares only the server's isolated network namespace to reach loopback; the host validates its output and alone writes the versioned manifest. The PR server receives no credentials, authority, Docker socket, or output mount. Prints the manifest on success. Exits 4 (trusted declaration malformed), 10 (subject head, authority head, run, repository, harness or subject root .dockerignore off vocabulary), 11 (checkout/image/workspace/governed journey/server/capture/output state UNKNOWN), 12 (subject or authority checkout is not its named exact head), 15 (capture invalid). A successful journey's later page errors remain publishable for fetch to classify as proven red on 13. This is an internal workflow interface with required --repository, not the reviewer-facing optional --repo contract and not an evidence import path.",
	),
);
