...
const post = leafCommand(
	...
	Command.withDescription(
		'Post the review-ui verdict on STDIN as ONE comment for this namespace — re-resolve the live head, read the evidence set through its manifest, independently re-render preview evidence or for CI evidence re-resolve the governed workflow/run/check/artifact ids through GitHub, re-validate every capture, verify-upload every capture BEFORE anything posts, record complete provenance, compose through the `verdict-marker` wire format, leak-scan, upsert, and read back. There is no --namespace. Prints one JSON object. Exits 3 (empty stdin), 4 (manifest/preview-recapture/receipt/declaration/identity malformed or design-harness.json invalid), 5 (machine-local path), 6 (bare @ reference), 7 (PR absent/closed), 8 (create/edit UNKNOWN), 9 (read-back mismatch), 10 (bad polarity/carrier), 11 (precondition or trusted provenance read failed), 12 (head/pixels stale), 13 (CI page crash cannot carry PASS), 15 (capture invalid), 17 (upload verification failed), 18 (the exact-head governed CI tuple required for revalidation is unavailable). Example: fabrika review-ui post 4321 --polarity FAIL --sha 03135b91 --clause "changes-requested" --evidence judged < verdict.md',
	),
);
