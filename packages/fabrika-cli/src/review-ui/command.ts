...
const fetch = leafCommand(
	...
	Command.withDescription(
		'Resolve the declared producer from the repository default branch, require its successful pull_request_target run at that exact authority revision, named check and unique non-expired artifact for the open PR\'s exact live head, validate the manifest and every PNG, then materialize the set in reviewer-owned scratch. There is no local path, workflow, run, check, artifact or manifest input. Preview rendering remains the default review-ui path. Machine stdout is exactly one JSON object shaped {"answer":"fetched","set":string,"pr":number,"head":string,"harness":string,"run":number,"artifact":number,"check":number,"surfaces":number,"captures":[{"surface":string,"path":string,"width":number,"height":number,"sha256":string,"pageErrors":{"rows":array,"more":number}}]}. Exits 4 (malformed declaration/manifest), 7 (PR absent/closed), 10 (unknown harness or bad set name), 11 (producer evidence unresolved), 12 (head moved), 13 (the integrity-validated set was materialized with an uncaught page error: proven red-render/FAIL ground, with every materialized capture path on stderr), 15 (capture hash/dimensions invalid). Example: fabrika review-ui fetch 7190 --harness tuval --out judged. Example success on a host whose temp root is /tmp: {"answer":"fetched","set":"judged","pr":7190,"head":"03135b91aa04f7e2c9d8b1640a5c22e9f01b7d3c","harness":"tuval","run":42,"artifact":51,"check":61,"surfaces":1,"captures":[{"surface":"tuval-cockpit-desktop","path":"/tmp/fabrika-review-ui/7190-03135b91/judged/captures/tuval-cockpit-desktop.png","width":1280,"height":800,"sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","pageErrors":{"rows":[],"more":0}}]}',
	),
);
