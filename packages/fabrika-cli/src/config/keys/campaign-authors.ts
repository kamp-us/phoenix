 ...
 *
 * A **narrowing predicate over the live ACL, never authority on its own**: an entry here still has
 * to hold `write` or above on the repository at the moment of the act. Its shipped default
 * is the empty set — nobody may declare — which is the one default this key can have, because the
 * privilege behind it is the dispatch permission itself, and a set that filled itself in
...
export const campaignAuthorsKey: KeyGroup<ReadonlyArray<GrantAuthor>> = {
	...
	jsonSchema: {
		type: "array",
		description:
			"Who may declare a campaign or flip its lifecycle state (`fabrika campaign open` / `fabrika campaign state`), narrowing the repository's collaborator ACL — an entry here still needs `write` or above on the repo. Each entry is a GitHub `@user` or `@org/team`, `@`-prefixed. Empty (or absent) means nobody may declare.",
