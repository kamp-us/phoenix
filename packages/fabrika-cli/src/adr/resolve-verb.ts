...
export const runResolve = (exec: Exec, options: ResolveOptions): VerbOutcome => {
	...
	const named: MergedRecord[] = merged.value.files.flatMap((file, i) => {
		const id = merged.value.ids[i];
	...
	for (const {id, file} of named) {
		...
		const record = readMergedRecord(exec, merged.value.sha, dir, file, id);
		if (record === null) {
			return refuse(FAILED, `adr resolve: cannot read ${dir}/${file} at ${merged.value.sha}`);
		}
		records.push(record);
	}
	...
	const scope = `adr resolve: scanned ${dir} at ${merged.value.sha}, ${merged.value.files.length} decision records; ${open.size} id(s) in flight across the open pull requests of ${repo}.`;
