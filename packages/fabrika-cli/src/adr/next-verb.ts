 ...
 * requests and with the ids claimed on this clone's own unpublished branches.
 *
 * The residual race is real and this verb does not close it: two authors between the same pair of
 * invocations still collide, and so do two lanes in *different* clones, whose branches never reach
 * one ref store. What the branch half does close is the shape that was hitting by default rather
