...
const guards = leafCommand(
	...
	Command.withDescription(
		"Scan the bound diff for anchored invariants it removes or modifies, and report the guard-bearing files it touches. Prints `guards\\t<hits|no-anchor-change|no-anchors-in-reach>\\t<anchors-in-reach>` then an `anchor` line per hit and a `guard-file` line per touched guard-bearing file, the guard-file evidence capped at five with a `guard-file-more` line carrying the remainder. no-anchors-in-reach is the scan reporting its own silence, never a clearance. Exits 7 (the PR is absent, closed, or empty), 10 (--sha is not a head SHA), 11 (the diff could not be read — UNKNOWN, never no-anchor-change), 12 (--sha is not the PR's head), 13 (the served diff body carries fewer files than git's own enumeration of the same range). A disagreement with GitHub's declared changed_files is printed on stderr and never refused on. Example: fabrika governance guards 4321",
	),
);
