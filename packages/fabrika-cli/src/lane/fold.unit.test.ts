...
import {CANCELLED_EVENT, CLEARED_EVENT, type CompiledLane, compile} from "./machine.ts";
...
describe("the cancellation terminal", () => {
	...
	it("ends a task parked in `blocked` — lane 5983's own shape", () => {
		...
		expect(applied).toMatchObject({
			...
			current: {stateValue: "cancelled", status: "done"},
		});
	});

	it("reaches every state a lane can sit in, `queued` included", () => {
		const compiled = lane(coderWorkflow());
		const applied = applyCancellation(compiled, statesOf(compiled, []), "issue", "duplicate", AT);

		expect(applied).toMatchObject({_tag: "Appendable", current: {stateValue: "cancelled"}});
	});
	...
	it("refuses a task sitting in a final whose outcome is already recorded", () => {
		...
		const applied = applyCancellation(compiled, statesOf(compiled, log), "task_a", "duplicate", AT);

		expect(applied).toMatchObject({_tag: "Refused"});
		expect(applied._tag === "Refused" && applied.reason).toContain("nothing here to cancel");
	});
	...
	it("cannot be hand-typed through `lane transition` — it is not an operator event", () => {
		...
		expect(applied).toMatchObject({_tag: "Refused"});
		expect(applied._tag === "Refused" && applied.reason).toContain("`lane cancel`");
	});
