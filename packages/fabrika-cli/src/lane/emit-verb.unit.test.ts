...
describe("lane emit", () => {
	...
	it("re-emits over a wrong-template lane whose every event is orphaned", async () => {
		const {out, fs} = await run(
			[
				[ISSUE, epic()],
				[SUBS, children],
		...
		expect(out.code).toBe(0);
		expect(fs.written.get(WORKFLOW)).toBe(golden());
		expect(JSON.parse(out.stdout)).toMatchObject({replaced: true, droppedEvents: 2});
		expect(out.stderr.join("\n")).toContain("orphaned event(s)");
	});

	it("refuses a booted lane whose log names a task the epic machine also carries", async () => {
		const {out, fs} = await run(
			[
				[ISSUE, epic()],
				[SUBS, children],
			...
			fakeFs({
				files: {
					[WORKFLOW]: coderTemplateText(),
					[LOG]: `${JSON.stringify({task: "issue_4301", event: "ISSUE_4301.WIP", at: AT})}\n`,
				},
				directories: [".fabrika/lanes/4300"],
			}),
		);
		...
		expect(out.code).toBe(LANE_EXISTS);
		expect(fs.written.size).toBe(0);
		expect(out.stderr.join("\n")).toContain("issue_4301");
		expect(out.stderr.join("\n")).toContain("history");
	});

	it("refuses an existing GENERATED machine — that one is regenerated by hand", async () => {
		const emitted = JSON.parse(coderTemplateText());
		emitted.id = "epic-4300";
		const {out, fs} = await run(
			[
				[ISSUE, epic()],
				[SUBS, children],
			...
			fakeFs({
				files: {[WORKFLOW]: JSON.stringify(emitted, null, "\t")},
				directories: [".fabrika/lanes/4300"],
			}),
		);
		...
		expect(out.code).toBe(LANE_EXISTS);
		expect(fs.written.size).toBe(0);
		expect(out.stderr.join("\n")).toContain("remove the directory");
	});
