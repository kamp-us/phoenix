...
const prove = leafCommand(
	...
	Command.withDescription(
		"Read the artifact a lane event claims — artifacts over self-reports, the rule the retired epic conductor held against a conducted branch, read off the board here because a lane owns none. Two events carry a claim: a DONE out of `build` claims an open PR whose body links the task's issue (or, for an investigation, the diagnosis comment a no-PR builder posted since the task entered build), and a PASS out of `review` claims a current-head verdict in every namespace that PR's diff derives, governance included. Every other event answers `not-required` at exit 0. Writes nothing — the append stays `lane transition`'s. Exits 4 (lane record read in full and not the shape), 7 (no lane there), 11 (a lane or board read failed — the proof is UNKNOWN, never proven), 13 (the task is not in the machine, or names no issue), 21 (the key is not a lane key), 22 (the artifact is provably not there), 23 (a required namespace has no current-head verdict — re-read, record nothing), 24 (a current-head FAIL under a claimed PASS), 25 (several open PRs link the issue). Example: fabrika lane prove 5673 DONE",
	),
);
