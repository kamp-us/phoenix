...
const assembly = leafCommand(
	...
	Command.withDescription(
		"Place the working tree an epic run assembles in — `epic/<n>` checked out at `.claude/worktrees/epic-<n>`, both derived from the epic number and never taken from the caller — and print its absolute path on stdout. The invoking checkout is NEVER switched. Idempotent in both directions: a worktree already holding the branch is resumed and its path answered with nothing written, and a branch that outlived its worktree — the state `--remove` at a terminal leaves behind — is checked out again as it stands, never re-cut off a fresh base. A worktree whose directory is gone but whose record git still carries (`prunable`) is that same state: the registration is cleared and the branch placed again, never answered as a live path. EVERY RESUME FETCHES FIRST and asks one question of the branch it found: is its head already contained in `origin/HEAD`. A multi-phase epic that shipped an intermediate tail lands in exactly that state — the branch holds nothing the trunk lacks and conflicts with everything the trunk took since, and until this read the verb could not tell it from an ordinary unlanded resume. A contained branch is re-cut off `origin/HEAD` in one `worktree add --no-track -B`, and the note says it re-cut and why; its seat, if it still has one, is dropped first WITHOUT `--force`, so git refusing to drop uncommitted work is what keeps unlanded bytes out of a re-cut. Containment is the whole warrant: a branch that is not contained resumes exactly as before, and an unreadable containment answer — a failed fetch, an `origin/HEAD` naming no commit — refuses at 11 rather than resolving either way. `--remove` is the lane's terminal step, fetches nothing and never forces — a dirty assembly tree is unlanded work, so git's refusal is the answer. Every mode reads the outcome back off `git worktree list` before answering. Exits 4 (the lane record was read in full and is not the shape), 7 (no lane there — emit the run's machine first), 8 (the placement or removal ran and did not read back, or a contained branch's seat would not drop — UNKNOWN), 11 (the working trees, the branches, the fetch or `origin/HEAD` could not be read — nothing was placed or removed), 33 (`epic/<n>` is checked out in the main working tree — switch that tree off it first), 39 (no .git entry exists at or above the cwd, so there is no owning repository from which to derive the default lanes root; an unreadable repository identity is UNKNOWN at 11; NOT \"no lane here\", so never a boot). Examples: fabrika lane assembly 5680 · fabrika lane assembly 5680 --remove",
	),
);
