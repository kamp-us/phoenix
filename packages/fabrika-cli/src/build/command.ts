...
const claimants = leafCommand(
	...
	Command.withDescription(
		'Read who holds the claim on an issue, holding none yourself. It takes no --token and needs no session, so a driver holding no claim of its own can read the claim state of one issue — confirm re-proves THIS lane\'s claim and refuses a token of any other session (see claude-plugins/fabrika/skills/build/contract.md). It writes nothing, and it clears nothing — a stranded claim leaves through "fabrika build adopt" then "fabrika build release", never through a TTL, a lease or an inference from absence (ADR 0295). Prints {"answer":"held"|"unclaimed","number":n,"holder":{"commentId","author","createdAt","token","session","authorized"}|null,"claimants":[…],"adopts":[…]}: the holder is the EARLIEST AUTHORIZED marker, the same one every ownership question resolves against, and every other marker is listed beside it — an unauthorized one is counted and named on stderr, never a winner (ADR 0055). A closed issue is answered rather than refused, because a marker outliving its issue is exactly the strandedness this reads for; its state is named on stderr. Exits 7 (the issue is proven absent — there is no thread to read), 11 (the issue, its comments or an author\'s permission could not be read — UNKNOWN, never "unclaimed"). Example: fabrika build claimants 6669',
	),
);
