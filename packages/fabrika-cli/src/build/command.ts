...
const reap = leafCommand(
	...
	Command.withDescription(
		'Sweep the registrations under .claude/worktrees/agent-* and classify each KEEP or REMOVE. REMOVE needs FOUR positive proofs together: the tree holds nothing uncommitted, it carries no lock, its HEAD is already on the trunk — reachable from origin/HEAD, or landed there as a squash, matched by comparing the patch id of what the HEAD adds against the trunk\'s own patches over exactly those paths — and the tree reads QUIET, its directory untouched for 24h. The subject of the third is the HEAD COMMIT, not a branch: the harness detaches the trees it registers, so a branch-keyed rule would judge almost none of them. The fourth is not a git fact and cannot be: an operator or reviewer SEAT drives its lane without ever committing or editing, so the three git proofs read a live seat as carrying nothing — the shape that removed one mid-drive. Any live signal, and any liveness read that failed, is KEEP, and the KEEP line names which signal held. EVERYTHING ELSE IS KEEP, per tree — dirty, locked, prunable, unlanded, live, and every read that failed — so one unreadable directory costs its own row and not the sweep. THE DEFAULT RUN MUTATES NOTHING: it prints the per-tree classification with its reason and stops; --execute is what removes. Each removal runs plain `git worktree remove` and NEVER --force, which is banned on every path; a removal git refuses leaves the tree registered and is reported, and every removal is read back off a second worktree list. The removal takes the tree, never the branch. Both halves of the report — what went and what was deliberately kept — are on stderr on every path, so a survivor is visible without re-running. Prints {"answer":"planned"|"reaped"|"none","executed":bool,"trunk":"origin/main","scanned":n,"removable"|"removed":[…],"kept":[…]}. Exits 8 (git refused a removal — the tree stays), 9 (git reported a removal and the registration survives, or the read-back failed), 11 (this run\'s own root, the registrations, or the trunk could not be read — nothing was removed). Example: fabrika build reap --execute',
	),
);
