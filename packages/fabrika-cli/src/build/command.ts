...
const resumeChild = leafCommand(
	"resume-child",
	{
		...
		token: tokenFlag.pipe(
			Flag.optional,
			Flag.withDescription(
				"the repair claim this lane already holds, when it is re-running — the claim step then answers off the standing marker and writes nothing; omit it on a first entry",
			),
		),
		repo: repoFlag,
	},
