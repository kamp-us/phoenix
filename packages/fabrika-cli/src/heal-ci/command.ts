...
const logs = leafCommand(
	...
	Command.withDescription(
		"Read the failed-job log for EVERY failing gating context at a head, not the first — an N-context red is N routed actions. First stdout line is `logs\\t<count>\\t<sha>`, then per context a `==== context <name> job <id> bytes <k> truncated <bool> ====` header, the log bytes, and an `==== end <name> ====` terminator, which is the framing `heal-ci classify` splits on. `logs 0 <sha>` is a proven answer: nothing gating is failing here. Which contexts block is the base branch's declared required set, read before anything is fetched: a failing context outside it is named on stderr and its log is never fetched, and a base branch declaring nothing falls back to the informational-name denylist. A required set that cannot be read is exit 11 naming that read as the cause, truncation is declared rather than silent, and a failing context with no workflow job behind it is emitted with an empty body rather than failing the whole read. Exits 7 (the PR or --sha commit is proven absent, or --context names no failing gating context here), 11 (the check runs, run list, a job log or the base branch's declared required set could not be read — UNKNOWN, never empty), 13 (an enumeration is provably short, or the base branch's ruleset walk never reached a terminal page), 15 (the platform reports the run's logs expired, which no retry changes). Example: fabrika heal-ci logs 4322 --sha 9fe12ab0",
	),
);
