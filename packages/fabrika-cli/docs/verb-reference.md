...
the `--class` is outside the review classes — on `lane open`, an off-set `class:<name>` label on the issue, refused before the document is placed · `39` the cwd is not in a repository · `40` another
...
**`triage apply --class` is the producer the lane class seed never had.** The lane machine reads
`context.<task>.classes` and routes `class:<name>`-guarded arms off it, and until this flag nothing
in the tree wrote that field: the committed coder template shipped `"classes": []`, `lane open`
copied it byte-identically, and every class a lane ever carried arrived on an event a head had
already raised off a diff — so a rendered-surface lane's *first* build ran in the plain `builder`
and reached `build:ui` only after a `review-ui` FAIL. The flag is repeatable, its vocabulary
is closed in code (`SHIP_CLASS_NAMES`, the same set `review scope` and `ship scope` partition a diff
into) rather than declared on the board, and an off-set spelling refuses on `10` before any label is
written — a `class:UI` on the board seeds a lane that matches no arm and routes as unclassed, the
failure with no symptom. `lane open` reads the label off the expectation read it already makes and
seeds the placed bytes; `lane emit` reads it off the `sub_issues` payload and seeds each child's
context entry, giving a `ui` child the `build:ui` / `review:ui` states the coder template already
had. Both refuse an off-set spelling before placement, and the compiler refuses one on read as the
backstop.
