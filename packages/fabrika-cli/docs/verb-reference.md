...
| `lane reconcile` | which lanes recorded a merge closure the board disagrees with, and the `<TASK>.CORRECTED` line that supersedes it — `--check` reports and appends nothing but costs the same one PR read per nominated lane, hundreds on a mature ledger; a read that proves no closure is `unknown`, never `closes`; ordered after `lane migrate`, which is what an `unmigrated` row names (ADR 0350) |
...
| `review ci` | the live check-run rollup at a head, fail-closed on incomplete enumeration; `--wait` bounds a `pending` one in-verb and prefixes `settle\t<settled\|budget-exhausted\|head-moved\|governance-owed>` |
