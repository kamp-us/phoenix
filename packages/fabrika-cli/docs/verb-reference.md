...
| `review-ui fetch` | the exact-subject-head GitHub Actions artifact for a governed localhost-only harness, admitted only when the run also matches the exact current default-branch authority revision and one paired PR/head association row, then validated and copied into reviewer-owned scratch; exit `13` materializes a proven red render and prints every capture path on stderr rather than routing CANT-SEE, and no local path or caller-selected producer input exists |
| `review-ui ci-produce` | the base-owned internal leg with required workflow identities (`--repository`, not reviewer `--repo`, plus `--authority-head`) that rejects a subject root `.dockerignore` before Docker can filter the exact-head context; its pinned, hook-disabled fetch runs no PR lifecycle code, then runs offline install/test in a disposable no-network workspace and serves a separate fresh exact-head workspace read-only with no network or host port; a base-owned capture sidecar shares only that isolated network namespace, and the host validates its captures and writes the manifest |
