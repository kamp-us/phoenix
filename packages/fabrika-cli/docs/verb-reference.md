...
| `review-ui ci-produce` | the base-owned internal leg with required workflow identities (`--repository`, not reviewer `--repo`, plus `--authority-head`) whose pinned, hook-disabled fetch runs no PR lifecycle code, then runs offline install/test in a disposable no-network workspace and serves a separate fresh exact-head workspace read-only with no network or host port; a base-owned capture sidecar shares only that isolated network namespace, and the host validates its captures and writes the manifest |
