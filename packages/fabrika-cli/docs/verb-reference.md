...
| `lane assembly` / `push` | an epic run's assembly worktree, and its published branch. Every resume fetches and asks whether the branch it found is already contained in `origin/HEAD` — the state a multi-phase epic lands in the moment it ships an intermediate tail. A contained branch is re-cut off the trunk in one `worktree add --no-track -B` and the note says it re-cut; its seat is dropped first without `--force`, so git refusing a dirty tree keeps unlanded bytes out of the re-cut. Anything less than containment resumes as it always did, and an unreadable answer is `11` |
