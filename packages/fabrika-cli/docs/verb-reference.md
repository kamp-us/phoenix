...
| `review-ui ci-produce` | the base-owned internal leg whose pinned, hook-disabled fetch runs no PR lifecycle code, then runs offline install/test in a disposable no-network workspace and serves a separate fresh exact-head workspace read-only with no network or host port; a base-owned capture sidecar shares only that isolated network namespace, and the host validates its captures and writes the manifest |
| `review-ui post` | the `review-ui` verdict on stdin, posted as one comment after a `review-ui render` receipt or CI receipt, plus capture/head validation and live GitHub workflow/run/check/artifact revalidation for CI evidence |
