...
| `lane assembly-pr` | the assembly PR's title and `## About this epic` section, derived from the epic issue — `--field title` prints `feat(epic): <the epic's own title>`, whose conventional type is `build/pr-title.ts`'s map rather than a second copy of the release-please rule, and `--field about` prints the section derived from the epic's `## Pitch` **Problem** paragraph — neutralised (a closing keyword swapped for a word GitHub's documented keyword list does not carry, the reference left as written), cut to its opening sentences under a word budget with `[…]` marking what a triage-length Problem left behind, and then re-read through `build pr`'s own body predicates so it cannot be a section that guard refuses. Bare values, one per call, because the operate skill's `gh pr create` fence interpolates them and derives nothing itself. An unpitched epic is an ANSWER — empty stdout, the reason on stderr — so a run still publishes its PR with no section. It opens and edits no pull request |
...
`56` the issue `lane assembly-pr` was pointed at carries no `type:epic` · `57` that verb's derived
About section still carries a stray closing keyword or a classification claim after neutralisation —
reword the epic's Problem paragraph, or write the section by hand.
