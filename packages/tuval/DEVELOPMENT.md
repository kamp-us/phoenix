...
standard pi agent directory when the variable is absent). Selecting a retained session first shows
acknowledged ownership with “geçmiş yükleniyor”. A bounded recent window then appears while the
separate Pi runtime may still say “çalışma zamanı yükleniyor”; Composer and controls become available
only after runtime readiness:
