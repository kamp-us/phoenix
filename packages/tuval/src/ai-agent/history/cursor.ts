import {anchorsCursor, type TranscriptItem} from "../ports/index.ts";

export type PageCursor =
	| {readonly kind: "page"; readonly before: string | null}
	| {readonly kind: "unavailable"};

/** The rule lives on the port, because `../history/window.ts` plans against it too (#9514). */
const isUnavailable = (item: TranscriptItem): boolean => !anchorsCursor(item);
