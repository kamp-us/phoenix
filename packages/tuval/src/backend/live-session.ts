...
export class PiLiveSession implements LiveSessionService {
	...
	static async connect(transportFactory: ByteTransportFactory): Promise<PiLiveSession> {
		return new PiLiveSession(await PiClient.connect({transportFactory}));
	}
	...
	attach = (sessionId: string): Promise<AttachLiveSessionOutcome> =>
			...
			try {
				...
				if (event._tag !== "diagnostic") {
					this.#events.push({
						_tag: "diagnostic",
						sequence: this.#nextSequence(),
						sessionId: this.#attachment?.snapshot.id ?? null,
						message: `Live-session subscriber failed: ${messageOf(error)}`,
					});
				}
			}
		}
