/**
 * The published surface of `@kampus/tuval-ui`, read off a real `pnpm pack` rather than off the
...
describe("the packed @kampus/tuval-ui", {timeout: SUBPROCESS_TEST_TIMEOUT_MS}, () => {
	beforeAll(() => {
		const tmp = mkdtempSync(join(tmpdir(), "tuval-ui-pack-"));
	...
	it("is public under its published name", () => {
		expect(manifest.name).toBe("@kampus/tuval-ui");
