 ...
 * the core in `apps/tuval/src/shell/core/machine.unit.test.ts`, because that is where the switch lives.
 */
...
describe("desk state", () => {
	...
	it("admits a desk it could have written and refuses anything else", () => {
		...
		expect([
			...
			// would be a second, quieter answer to the same question (`apps/tuval/src/shell/program.ts`).
