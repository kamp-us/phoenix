...
export interface ChatWindowOptions {
	 ...
	 * no route to the shell (`apps/tuval/src/page/renderers.tsx`). A window built without one draws no kernel
...
function ChatWindow({
	...
	const commit = useCallback((next: (current: ChatView) => ChatView) => {
		...
		// rebuilds `ShellState` wholesale (`apps/tuval/src/shell/core/machine.ts`), so a no-change write from
	...
	/**
	 * `<c-b> a`, arriving as the key that chord's command row mints (`apps/tuval/src/shell/commands/table.ts`). The
