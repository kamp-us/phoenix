...
/** The SDK's own failures module, found through its exports map rather than a path into `packages/`. */
const sdkFailures = createRequire(import.meta.url).resolve(
	"@kampus/tuval-sdk/kernel/ai-agent/core/failures",
);
...
describe("chat window boundary", () => {
	...
	it("writes only failure tags the core's own refusals declare", () => {
		const failures = readFileSync(sdkFailures, "utf8");
