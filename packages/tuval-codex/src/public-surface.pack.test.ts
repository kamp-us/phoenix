/**
 * The published surface of `@kampus/tuval-codex`, read off a real `pnpm pack` rather than off the
 ...
 */
const DOORS = [".", "./window", "./package.json"];
...
describe("the packed @kampus/tuval-codex", {timeout: SUBPROCESS_TEST_TIMEOUT_MS}, () => {
	beforeAll(() => {
		const tmp = mkdtempSync(join(tmpdir(), "tuval-codex-pack-"));
	...
	it("is public under its published name", () => {
		expect(manifest.name).toBe("@kampus/tuval-codex");
