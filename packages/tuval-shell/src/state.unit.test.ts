...
describe("what a shell calls itself", () => {
	it("titles itself by its id and the basename of where it runs", () => {
		...
		expect(titleLine({...idle, id: "fetcher", cwd: "/tmp/"})).toBe("fetcher · tmp");
	});
...
describe("what a shell says it is doing", () => {
	...
	it("names the command while one is up", () => {
		expect(statusLine({...idle, running: request})).toBe("running git fetch --all");
	});
	...
	it("reports the exit code and how long it took", () => {
		...
		expect(statusLine(ran({ending: {_tag: "exit", code: 1}}))).toBe("exit 1 in 1.2s");
	});

	it("tells a timeout, a kill and a restart apart, because they are three different failures", () => {
		expect(statusLine(ran({ending: {_tag: "timeout"}}))).toBe("timed out after 1.2s");
		expect(statusLine(ran({ending: {_tag: "killed"}}))).toBe("killed after 1.2s");
		expect(statusLine(ran({ending: {_tag: "interrupted"}, durationMs: 0}))).toBe(
