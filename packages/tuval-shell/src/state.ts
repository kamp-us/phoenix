...
export const commandHead = (command: string): string => {
	...
	return line.length <= COMMAND_HEAD ? line : `${line.slice(0, COMMAND_HEAD - 1)}…`;
};
...
export const titleLine = (state: ShellState): string => `${state.id} · ${basename(state.cwd)}`;
...
export const boundedTail = (
	...
	return droppedNote(alreadyDropped + start) + decoder.decode(bytes.subarray(start));
};
