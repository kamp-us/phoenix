...
export default defineConfig({
  resolve: {
    ...
    alias: {
      "@kampus/tuval-workspace": fileURLToPath(
    ...
    // built by one is a stranger to a decoder from the other, which is how `@kampus/tuval-cron`
    // found it, with `:cron run`'s args decoding to `Symbol()` instead of `{}`.
