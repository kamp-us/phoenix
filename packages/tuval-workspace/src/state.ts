/**
 * Workspace's state, and everything that is a pure function of it — the leaf both halves of this
 * package share and neither half owns.
 *
 * **Why it is a leaf.** `./workspace.ts` reaches `@kampus/tuval/authoring`, which reaches the
 ...
 * path to `workspace.ts`. This is the split `@kampus/tuval-cron` drew between its `state.ts` and
 ...
 * state: the config chose them at `workspace(...)` and nothing that happens to a workspace moves
 ...
 * answers and a handler runs (`./workspace.ts`), so nothing reads this to decide what to *do*; what
 ...
 * off every workspace, which is the honest place for a fact that belongs to no row.
 *
...
/**
 * How many workspaces this program will hold at once. Bounded because an unbounded map in state is
 ...
 * for, and then the newest live workspace spelled out, because one line can hold exactly one.
 *
...
/**
 * The events the window's controls send: the same arrivals `:workspace open` and
 * `:workspace close` put on the two in-ports, because a window and a spell asking for the same
