...
const drive = (program: Authored) =>
  testProgram(program as unknown as Drivable);
...
}): ReadonlyArray<{ readonly type: string }> =>
  run.effects as ReadonlyArray<{ readonly type: string }>;
...
const jobRef = workspaceProgram({ ...options, job: {} as ShapeSource }).args
  .job;
...
const withJob = () =>
  workspaceProgram({ ...options, job: session() as ShapeSource });
...
describe("a workspace program says what it is before anything has happened", () => {
  ...
  it("refuses an id that is not a word, at the config call rather than at boot", () => {
    expect(() => workspaceProgram({ ...options, id: "" })).toThrow(
      /non-empty word/,
    );
    expect(() => workspaceProgram({ ...options, id: "my workspace" })).toThrow(
      /no spaces/,
    );
  });

  it("refuses an empty repo and a backwards port range where they are written", () => {
    expect(() => workspaceProgram({ ...options, repo: "  " })).toThrow(
      /cannot be empty/,
    );
    expect(() =>
      workspaceProgram({ ...options, port: { from: 5199, to: 5170 } }),
    ).toThrow(/must run upwards/);
  });
});

describe("open", () => {
  const opened = () =>
    drive(workspaceProgram(options)).send("open", { name: "feature-x" });
  ...
  it("asks for nothing at all when it refuses, beyond writing the refusal down", () => {
    ...
    expect(asked(refusal).map((effect) => effect.type)).not.toContain(
      "workspace.provision",
    );
  });
  ...
  it("says on the tile what it is doing, by name", () => {
    expect(opened().effects).toContainEqual(
      emit(STATUS_PORT, "0 open · provisioning feature-x"),
    );
  });

  it("carries the branch prefix the config chose", () => {
    const run = drive(
      workspaceProgram({ ...options, branchPrefix: "build/" }),
    ).send("open", { name: "9287" });
  ...
  it("refuses a second open of a name it already holds, and says so", () => {
    ...
    expect(again.state.refusals).toEqual([
      { name: "feature-x", reason: "duplicate", at: SEVEN },
    ]);
  });
  ...
  it("refuses a name a branch and a directory cannot both be called, and says so", () => {
    ...
    expect(run.state.refusals.map((one) => one.reason)).toEqual([
      "name",
      "name",
      "name",
    ]);
  ...
  it("refuses a second open while one is still provisioning, and says so", () => {
    ...
    expect(second.state.refusals).toEqual([
      { name: "bugfix", reason: "busy", at: SEVEN },
    ]);
    expect(second.effects).toContainEqual(
      emit(
        STATUS_PORT,
        "0 open · provisioning feature-x · refused bugfix: busy",
      ),
    );
  });
...
describe("provisioned, and the agent inside", () => {
  ...
  it("says `1 open · feature-x :5174 idle` until an agent is up", () => {
    expect(ready().effects).toContainEqual(
      emit(STATUS_PORT, "1 open · feature-x :5174 idle"),
    );
  });
  ...
  it("tells the session where it is, because a spawn could not (phoenix#9287)", () => {
    ...
    expect(
      portsOf(session()).prompt?.accepts(
        (sent as { readonly payload: unknown }).payload,
      ),
    ).toBe(true);
  ...
  it("keeps a reply that arrived with no agent up at all, rather than dropping it", () => {
    ...
    expect(none.state.unattributed).toEqual([
      { text: "from nowhere", at: SEVEN },
    ]);
  });
  ...
  it("says `running` once an agent is up, and keeps the session after a turn", () => {
    ...
    expect(up.effects).toContainEqual(
      emit(STATUS_PORT, "1 open · feature-x :5174 running"),
    );
...
describe("the seam — the compiled row and the handlers spread onto it", () => {
  ...
  it("answers every effect this program names, and keeps the six the kernel wrote", () => {
    ...
    expect(keys).toEqual(
      expect.arrayContaining(["emit", "spawn", "send", "ask", "reply", "stop"]),
    );
  });

  it("answers them on the row with a job filled too, which is the other branch", () => {
    const keys = Object.keys(
      workspace({ ...options, job: session() as ShapeSource }).handlers,
    );
    expect(keys).toEqual(
      expect.arrayContaining([
        "workspace.provision",
  ...
  it("runs those handlers over the `Runner` the row was configured with", async () => {
    ...
    await Effect.runPromise(
      row.handlers["workspace.teardown"](effect) as Effect.Effect<unknown>,
    );
...
describe("the effect a cell answered, run by the handler that takes it", () => {
  ...
  it("provisions the plan the `open` cell derived and answers `provisioned`", async () => {
    ...
    const events = await perform(
      withRunner,
      only(queued, "workspace.provision"),
    );
    expect(events).toEqual([
      { type: "provisioned", name: "feature-x", port: 5170 },
    ]);
  ...
  it("tears down the plan the `close` cell derived, with no `--force` anywhere", async () => {
    ...
    const events = await perform(
      withRunner,
      only(closed, "workspace.teardown"),
    );
...
describe("a refused write, through the whole chain", () => {
  ...
  it("records the failure at the env step rather than going quiet", async () => {
    ...
    expect(run.state.workspaces[0]).toMatchObject({
      ...
      status: "failed",
      detail:
        "env: could not write /repo/.workspaces/feature-x/.env: EROFS: read-only file system",
    });
  });
...
describe("close", () => {
  ...
  it("stops the agent first, and asks for nothing else until that ending lands", () => {
    ...
    expect(asked(run).map((effect) => effect.type)).not.toContain(
      "workspace.teardown",
    );
  });
  ...
  it("still removes the worktree when the agent was already gone", async () => {
    ...
    const withRunner = { ...options, runner };
    const gone = drive(
      workspaceProgram({ ...withRunner, job: session() as ShapeSource }),
    )
      .send("open", { name: "feature-x" })
      .event({ type: "provisioned", name: "feature-x", port: 5174 })
      .event({ type: "spawned", process: agent, program: "claude-session" })
  ...
  it("waits for the ending of the agent it named, not any agent that happens to end", () => {
    ...
    expect(asked(two).map((effect) => effect.type)).not.toContain(
      "workspace.teardown",
    );
    expect(
      two.event({ type: "stopped", process: agent }).effects,
    ).toContainEqual(
      teardownEffect(
        closePlan(
          settle(options),
  ...
  it("keeps the record as failed when a teardown command refused, holding the reason", () => {
    ...
    expect(run.state.workspaces[0]).toMatchObject({
      status: "failed",
      detail:
        "teardown: dropdb --if-exists app_feature-x: database is being accessed",
    });
  });
...
describe("a workspace program restarted", () => {
  ...
  it("asks to reconcile on every restore, holding a job or not", () => {
    expect(withJob().resume(interrupted().state)).toEqual([
      { type: "restored" },
    ]);
    expect(
      workspace({ ...options, job: session() as ShapeSource }).resume?.(
        interrupted().state,
      ),
  ...
  it("answers one `workspace.reconcile`, naming every record it just settled", async () => {
    ...
    const events = await Effect.runPromise(
      workspaceHandlers(settle({ ...options, runner }))["workspace.reconcile"](
        effect,
      ),
    );
    expect(events).toEqual([{ type: "reconciled", missing: ["bugfix"] }]);
    expect(
      events.reduce((step, event) => step.event(event), run).state.workspaces,
    ).toContainEqual(
  ...
  it("records a workspace whose directory somebody removed as `gone`, and keeps it", () => {
    ...
    const feature = run.state.workspaces.find(
      (one) => one.name === "feature-x",
    );
  ...
  it("leaves a workspace whose directory is still there exactly where it was", () => {
    ...
    expect(
      run.state.workspaces.find((one) => one.name === "feature-x")?.status,
    ).toBe("open");
  });
...
describe("the two spells", () => {
  it("`open` sends to its own program's `open` port and asks for nothing else", () => {
    const run = drive(workspaceProgram(options)).call(
      "open",
      { name: "feature-x" },
      scope,
    );
...
describe("the job shape against a real session row", () => {
  ...
  it("is fitted by the live row itself, which is why no wrapper stands here", () => {
    ...
    expect(() =>
      workspace({ ...options, job: session() as ShapeSource }),
    ).not.toThrow();
    expect(() =>
      workspace({
        ...options,
...
describe("a user's `.tuval/tuval.config.ts`", () => {
  it("builds a workspace row through the package's entry, with the id the graph node names", () => {
    ...
    expect(config.graph.nodes.map((node) => node.program)).toContain(
      "workspace",
    );
  });
