...
describe("the status line", () => {
  ...
  it("says `running` for a workspace holding an agent", () => {
    expect(
      statusLine(state({ workspaces: [record({ agent: "proc-a" as never })] })),
    ).toBe("1 open · feature-x :5174 running");
  });
  ...
  it("says the newest refusal, because a refusal that says nothing is a button that did nothing", () => {
    expect(
      statusLine(state({ refusals: [refusal({ reason: "limit" })] })),
    ).toBe("0 open · nothing open · refused feature-x: limit");
    // An empty name is one of the four refusals, so it gets a word rather than a blank.
    expect(
      statusLine(state({ refusals: [refusal({ name: "", reason: "name" })] })),
    ).toBe("0 open · nothing open · refused (unnamed): name");
  });
  ...
  it("does not count a gone or failed workspace as open, and says which it is", () => {
    expect(
      statusLine(state({ workspaces: [record({ status: "gone" })] })),
    ).toBe("0 open · nothing open");
    expect(
      statusLine(state({ workspaces: [record({ status: "failed" })] })),
    ).toBe("0 open · nothing open");
  });
});
...
describe("the bookkeeping helpers", () => {
  ...
  it("calls provisioning, open and closing live, and gone and failed not", () => {
    expect(
      (["provisioning", "open", "closing", "failed", "gone"] as const).map(
        (status) => isLive(record({ status })),
      ),
...
describe("the window's view", () => {
  ...
  it("shows an em dash where there is no port yet", () => {
    expect(
      workspaceView(
        state({ workspaces: [record({ port: null, status: "provisioning" })] }),
      ).rows[0]?.port,
    ).toBe("—");
  });
  ...
  it("refuses another open at the bound", () => {
    const full = Array.from({ length: LIMIT }, (_, index) =>
      record({ name: `w${index}` }),
    );
    expect(workspaceView(state({ workspaces: full })).canOpen).toBe(false);
    expect(workspaceView(state({ workspaces: full.slice(1) })).canOpen).toBe(
      true,
    );
  });
  ...
  it("spells a refusal out, where the status line only had room for one word", () => {
    const view = workspaceView(
      state({ refusals: [refusal({ name: "", reason: "name" })] }),
    );
  ...
  it("shows the unowned replies with the one line saying why they are unowned", () => {
    const view = workspaceView(
      state({ unattributed: [{ text: "done, two files", at: SEVEN }] }),
    );
    expect(view.unattributed).toEqual([
      { key: `${SEVEN}-0`, text: "done, two files" },
    ]);
...
describe("the predicate the page admits a state through", () => {
  ...
  it("refuses a state whose refusal or unattributed list is not one", () => {
    expect(isWorkspaceState({ ...state(), refusals: [{ name: "x" }] })).toBe(
      false,
    );
    expect(
      isWorkspaceState({
    ...
    expect(
      isWorkspaceState({ ...state(), unattributed: [{ text: 7, at: 1 }] }),
    ).toBe(false);
  });
  ...
  it("refuses a record whose status is not one of the five", () => {
    expect(
      isWorkspaceState(
        state({ workspaces: [record({ status: "sleeping" as never })] }),
      ),
    ).toBe(false);
  });
});
...
describe("the events the window's controls send", () => {
  ...
  it("keeps the forcing one a different event with a different name", () => {
    ...
    expect(closeEvent("feature-x").type).not.toBe(
      discardEvent("feature-x").type,
    );
  });
