...
The package exists so web and Tuval consume one implementation instead of copying primitives or reaching across application boundaries. Issue [#7561](https://github.com/kamp-us/phoenix/issues/7561) records the ownership move from `apps/web`; the move keeps the existing component contracts and rendered surface intact while making the design law portable to another consumer.
