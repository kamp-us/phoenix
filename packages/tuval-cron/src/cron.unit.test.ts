...
describe("cron, reporting", () => {
	...
	it("announces the whole turn on `brief`, not the line the tile keeps", () => {
		const run = finished(true);
		expect(run.effects).toContainEqual(emit(BRIEF_PORT, turn("five lines\nand the rest", true)));
	...
	it("says nothing on `brief` for a run with no turn behind it", () => {
		...
		expect(
			died.effects.filter((effect) => effect.type === "emit" && effect.port === BRIEF_PORT),
		).toEqual([]);
...
describe("cron, restarted mid-run", () => {
	...
	it("announces no brief for the run the restart cut in half, because there was no turn", () => {
		expect(
			restarted().effects.filter((effect) => effect.type === "emit" && effect.port === BRIEF_PORT),
		).toEqual([]);
		// And the late answer from the old child, which *is* a turn, leaves through the port as usual.
		expect(restarted().event({type: "result", payload: turn("late", true)}).effects).toContainEqual(
			emit(BRIEF_PORT, turn("late", true)),
		);
	});
...
describe("cron's job shape against a real session row", () => {
	...
	it("is fitted by the live row itself, which is why no wrapper stands here", () => {
		expect(() => cron({...options, job: session() as ShapeSource})).not.toThrow();
		expect(() => cron({...options, job: {id: "not-a-job", ports: {}} as ShapeSource})).toThrow();
	});
...
describe("a user's `.tuval/tuval.config.ts`", () => {
	...
	it("carries a second, named cron beside it without either colliding", () => {
		...
		expect(config.programs.map((row) => row.id)).toEqual(["cron", "evening-summary"]);
		expect(config.graph.nodes.map((node) => node.id)).toEqual(["cron", "evening-summary"]);
	});
	...
	it("gives every cron its own `brief`, so two crons are two routes", () => {
		...
		expect(portsOf(standup)[BRIEF_PORT]?.kind).not.toBe(portsOf(eveningSummary)[BRIEF_PORT]?.kind);
	});
...
describe("more than one cron in a config", () => {
	...
	it("gives each row its own program id, which is its graph node and its spell", () => {
		...
		expect([morning.id, evening.id]).toEqual(["morning-brief", "evening-summary"]);
	...
	it("names the cron on its own tile, so two tiles are not both `cron · …`", () => {
		expect(named("morning-brief", "0 7 * * *").label).toBe("morning-brief (claude-session)");
		expect(
			testProgram(
				cronProgram({
					id: "morning-brief",
